#!/bin/bash
# gbx:coverage:python-docs - Run Python documentation tests with code coverage
...
show_help() {
    show_banner "📊 GeoBrix: Python Docs Coverage"
    echo -e "${CYAN}Usage:${NC}"
    echo -e "  ${GREEN}gbx:coverage:python-docs${NC} ${YELLOW}[options]${NC}"
    echo ""
    echo -e "${CYAN}Options:${NC}"
    echo -e "  ${GREEN}--path <dir>${NC}              Specific test directory (relative to docs/tests/python/)"
    ...
    echo -e "  HTML Report:  ${YELLOW}docs/tests/coverage-report/index.html${NC}"
    echo ""
    echo -e "${CYAN}Examples:${NC}"
    echo -e "  ${YELLOW}gbx:coverage:python-docs${NC}"
    echo -e "  ${YELLOW}gbx:coverage:python-docs --min-coverage 80 --open${NC}"
    echo -e "  ${YELLOW}gbx:coverage:python-docs --path api/ --open${NC}"
    echo ""
}

# Parse arguments
TEST_PATH="/root/geobrix/docs/tests/python/"
MIN_COVERAGE=""
LOG_PATH=""
OPEN_REPORT=false
...
while [[ $# -gt 0 ]]; do
    case $1 in
        --path)
            TEST_PATH="/root/geobrix/docs/tests/python/$2"
            shift 2
            ;;
        --min-coverage)
...
# Coverage measures source code in python/geobrix/src/, not docs tests
PYTEST_CMD="unset JAVA_TOOL_OPTIONS && \
    cd /root/geobrix && \
    python3 -m pytest $TEST_PATH \
    -v --tb=short \
    --cov=/root/geobrix/python/geobrix/src/databricks/labs/gbx \
    --cov-report=html:/root/geobrix/docs/tests/coverage-report \
    --cov-report=term \
    -m 'not integration' \
    $MIN_COVERAGE"
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    echo -e "  HTML: ${YELLOW}docs/tests/coverage-report/index.html${NC}"
    
    if [ "$OPEN_REPORT" = true ]; then
        echo ""
        open_report "$PROJECT_ROOT/docs/tests/coverage-report/index.html"
    fi
else
