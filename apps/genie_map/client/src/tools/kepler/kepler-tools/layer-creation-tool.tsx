...
function isAddLayerArgs(args: unknown): args is AddLayerArgs {
...
function isAddLayerFunctionContext(context: unknown): context is AddLayerFunctionContext {
  return context !== null && typeof context === 'object' && 'getDatasets' in context && typeof (context as AddLayerFunctionContext).getDatasets === 'function';
}
...
async function executeAddLayer(args: any, options: any): Promise<ExecuteAddLayerResult> {
  try {
    ...
    if (!layer) {
      // for point layer, try to create a point layer manually if LLM suggests Lat/Lng fields
    ...
    const columns = (layer as { config?: { columns?: Record<string, { value: string }> } })?.config?.columns || {};
    ...
    if (colorBy) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const colorField = dataset.fields.find((f: any) => f.name === colorBy);
      if (!colorField) {
...
export function AddLayerToolComponent({layer, datasetId}: {layer: object; datasetId: string}) {
