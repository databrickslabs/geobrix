...
// Define the state shape for kepler-demo (uses demo namespace)
interface KeplerDemoState {
  demo: {
    keplerGl: {
      map: {
        visState: {
...
export function LisaToolComponent(props: LisaAdditionalData) {
  const datasets = useSelector((state: KeplerDemoState) => state.demo?.keplerGl?.map?.visState?.datasets);
  const layers = useSelector((state: KeplerDemoState) => state.demo?.keplerGl?.map?.visState?.layers);
  ...
  useEffect(() => {
    ...
    if (newDataset) {
      ...
      const colorLegends: Record<string, string> = {};
      for (let i = 0; i < colors.length; i++) {
        colorLegends[colors[i]] = labels[i];
      }
      const colorMap = colors.map((color, index) => [index, color]);
      const layerConfig = layer.config as { columns: Record<string, { value: string }>; visConfig: Record<string, unknown> };
      const newLayer = {
        id: layerId,
        type: layer.type,
        config: {
          ...layer.config,
          dataId: datasetId,
          columns: Object.keys(layerConfig.columns).reduce((acc, key) => {
            acc[key] = layerConfig.columns[key].value;
            return acc;
          ...
          visConfig: {
            ...
            colorRange: {
              ...(layerConfig.visConfig.colorRange as object),
              colorDomain,
              colors,
              colorMap,
              colorLegends
