...
# Needs a Planetary Computer token available at stage time. AOI + window are the
# knobs; the default is a short window over a CH4 hotspot region.
readers.stage_netcdf_corpus(spark, f"{CORPUS}/netcdf",
                            bbox=[-104.5, 31.0, -101.5, 33.0],
                            temporal="2021-01-01/2021-01-08")
```

Staging is **decoupled from the read** — the token is only needed once, at stage time; the
benchmark itself just globs the staged pool. If the pool is empty or missing (no token, or
staging not yet run), the NetCDF benchmark **skips cleanly** with a clear message rather
