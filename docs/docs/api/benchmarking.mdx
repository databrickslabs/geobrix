...
**Conclusion:** FILE is not a throughput win for virtual-tile reads. The bottleneck is
per-tile HTTP stream setup and COG-header parsing over HTTP — each tile opens a new
`FileRef` HTTP connection and re-parses the COG header before reading the window bytes.
GDAL windowed reads over FUSE use block-level I/O against the OS page cache, which wins
regardless of reuse pattern. FUSE does **not** stage the whole file; it reads only the
256px COG blocks covering each window, the same data volume as FILE, but without the
per-tile HTTP overhead. FILE remains useful for governed file access on clusters without a
FUSE mount; it is not a raster read optimization.
