...
- **[FILE-on ≈ FILE-off on dedicated clusters](#file-capability-cog-multiwindow)** — FILE is governed file access, not a per-tile read optimization. Grouping (open amortization) is the performance lever. Serverless supports FILE-column-table reads (verified on environment v6, ~2 s per 1,000 tiles); the directory `raster_gbx` DataSource path is slower there — see [capability tiers](../readers-writers#capability-tiers).
...
For a comparison of the full read paths — Delta-scan FILE-column-table (~2 s), directory enumeration (~10 s/10k files), and DataSource per-tile FUSE open (~20 s classic / ~74 s Serverless per 1k tiles) — see the [capability tiers & read-options matrix](../readers-writers#capability-tiers). The fast Serverless path is reading from a FILE-column table (Delta scan).
