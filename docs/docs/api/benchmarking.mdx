...
This is a **how-to-measure** section. No results table is provided here: the FILE
byte-range benefit is COG-size-dependent, and a representative measurement requires
a file size that matches your workload. An earlier run on an 18 MB COG showed FILE
**~5% slower** than FUSE — the per-window `FileRef` minting overhead dominated at that
size. The benefit grows as the COG grows: FUSE stages the **entire file** to local disk
before each window read; FILE fetches **only the COG tile blocks** that cover the window.
Measure on a COG that represents your real files.
