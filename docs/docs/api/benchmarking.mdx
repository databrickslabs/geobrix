...
These four functions are **lightweight-only** (no heavyweight counterpart) and do not
appear in the parity comparison tables. They belong to the **point-cloud-to-raster**
shape family: binning scattered xyz points onto a pixel grid (`rst_binpoints`/`rst_binpoints_agg`),
polygon-extraction per value band (`rst_isoband`), and DSM/DEM subtraction (`rst_chm`).
