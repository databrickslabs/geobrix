...
- **Discrete-grid aggregation (`rst_h3_*`/`rst_quadbin_*`)** is a ~1.7–6× lightweight win and matches within tolerance.
  ...
  >
  > Representative pure-core timings from that run (512² · 4-band · `float32`; the BNG functions on an EPSG:27700 British-National-Grid tile, quadbin on EPSG:4326): `rst_bng_tessellate` is a **14.6× lightweight win** (heavy 1.56 s vs light 0.11 s, exact) and `rst_quadbin_tessellate` a **12.9× win** (1.23 s vs 0.10 s, exact). The five BNG reducers, now binning real cells, are **heavyweight-favored** — roughly 0.12–0.15× (heavy ~1.1–1.4 s vs light ~8.9–9.0 s) — because the lightweight tier's per-pixel reprojection-plus-bin over a full raster is dominated by the EPSG:27700 warp and Python-side binning, work the JVM does in-process. The **BNG reducers and `rst_bng_tessellate` reproject the tile to EPSG:27700 internally**, so their timing includes that warp — unlike the 4326-native H3 and quadbin functions — and a like-for-like reading should account for it. The reducers and tessellators are registered on the lightweight tier as table functions (invoked with SQL `LATERAL`); their at-scale spark-path timings are in the [raster→grid family subsection below](#raster-grid-family-at-scale).
...
| `rst_bng_rastertogridmedian` | 40.27 | 216.76 | 0.19× | timing-only |
| `rst_bng_rastertogridmax` | 39.36 | 225.70 | 0.17× | timing-only |
| `rst_bng_rastertogridstddev` | 37.58 | 227.07 | 0.17× | timing-only |
| `rst_bng_rastertogridavg` | 36.56 | 226.50 | 0.16× | timing-only |
| `rst_bng_rastertogridcount` | 37.94 | 243.05 | 0.16× | timing-only |
| `rst_bng_rastertogridmin` | 37.54 | 235.50 | 0.16× | timing-only |
| `rst_bng_rastertogridvariance` | 37.85 | 229.99 | 0.16× | timing-only |
| `rst_bng_rastertogridsum` | 38.33 | 253.64 | 0.15× | timing-only |

The tessellators are the standout lightweight wins (`rst_quadbin_tessellate` 9.5×, `rst_h3_tessellate` 6.1×): the covering-cell walk is cheap in Python and the `LATERAL` fan-out amortizes the JVM↔Python boundary once per input tile. The quadbin reducers sit at slight lightweight advantage to parity (~1.1–1.8×), while the H3 reducers are modestly heavyweight-favored (~0.6–0.8×) — H3's finer resolution 7 bins more cells per tile, so the per-cell Python work outweighs the compute saving at scale. The **BNG reducers are the most heavyweight-favored (~0.15–0.19×)** because the lightweight tier reprojects each full raster to EPSG:27700 and bins per-pixel in Python — work the JVM does in-process — so the 27700 warp dominates their at-scale cost (they bin **real Great Britain cells** on an EPSG:27700 tile over central London, not an empty grid). The three grid rasterize aggregators are heavyweight-favored on their small burned canvas (~0.63–0.70×) and match **exactly** across tiers.
