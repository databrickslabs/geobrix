...
- **[Grouping amortizes open cost](../api/performance#virtual-tile-read-performance)** — windowed COG reads from the same source open once per partition (grouped executor). With `ORDER BY path` or repartitioning, byte-range stream reads are **10–290× faster** than FUSE per window; without amortization, per-tile opens grow prohibitive with file size.
- **[FILE-on ≈ FILE-off on dedicated clusters](#file-capability-cog-multiwindow)** — FILE is governed file access, not a per-tile read optimization. Grouping (open amortization) is the performance lever. Serverless: the FILE-column Delta-table read path is **~16–17× faster** than a directory scan (~2 s vs. ~30 s per 1,000 tiles, verified on environment v6); the directory `raster_gbx` DataSource path is slower there — see [capability tiers](../readers-writers#capability-tiers) and [Serverless & Memory](../serverless-and-memory).
...
- **[Geometry-aware expansion scales linearly](#results)** — `geomkring`/`geomkloop` grow grid rings from a geometry: on Serverless, 10,000 notional building footprints expand to h3 res-10 rings (`k=10`) in ~7.9 s (~1,260 buildings/s), and 100,000 footprints reach 35.7M cells in ~72 s. h3 leads the four grids; see the Grid tab.
...
| Lightweight (pyrx) | x86 **or ARM** | The `[light_env6]` wheel (Serverless) or `[light_dbrN]` (classic) |
...
The comparison `summary.md` opens with **insights** — the biggest wins and a consistency tally (e.g. *7 compared cells: 0 exact, 7 within-tol, 0 divergent*) and the tile scale (*1,000 tiles/iteration, spark-path — every tile processed each timed iteration*) — followed by a per-function table. For a spark-path run it looks like:
...
<Tabs groupId="bench-results" className="gbx-bench-tabs">
...
### Pure-core (local, 1024²) {#pure-core}
...
#### Combine family (`rst_combinemin`, `rst_combinemax`, `rst_combinemedian`, `rst_combinesum`, `rst_combinestddev`, `rst_combinecount`)

The six raster-reduction functions that reduce an aligned stack of tiles to a single output tile (min/max/median/sum/stddev/count across the stack) ran in a separate cluster run alongside `rst_align_to`:

> **Environment:** Databricks · DBR 18.x · 20 workers · 1,000 tiles per iteration · lightweight tier with ZSTD-L6 default.

**Consistency:** All compared cells were **exact or within tolerance (0 divergent)**.

The combine family's ~9–12× light gap is dominated by per-tile Python GDAL decode and encode; the ZSTD-L6 default reduced it from the previous ~15–18× by cutting encode time. Setting `GBX_ZSTD_LEVEL=fast` (L1) trades a ~1–2% size increase for additional encode speed.

| Function | Heavy/tile (ms) | Light/tile (ms) | Speedup | Consistency |
|---|---|---|---|---|
| `rst_combinemin` | 3.10 | 34.0 | 0.09× | within_tol† |
| `rst_combinemax` | 3.47 | 30.1 | 0.12× | within_tol† |
| `rst_combinemedian` | 3.74 | 44.5 | 0.08× | within_tol† |
| `rst_combinesum` | 3.25 | 28.3 | 0.11× | within_tol† |
| `rst_combinestddev` | 3.01 | 33.1 | 0.09× | within_tol† |
| `rst_combinecount` | 2.76 | 24.3 | 0.11× | within_tol† |

<sub>† Aggregate result — 0 divergent across the six functions; per-function exact-vs-within-tolerance breakdown not available for this run.</sub>

`rst_align_to` was benchmarked in the same run (timing-only; no cross-tier output fingerprint — the output geometry varies by reference tile and CRS).
...
> see the [grid raster tessellation explainer](./grid-raster-tessellation)) — and timings will be added on the next
> cluster run.
...
**The quadbin cell-math functions were benchmarked on a cluster** (DBR 17.3 LTS, x86, spark-path, 1,000
tiles/iteration, 5 measured iterations, both tiers). **Exact cell-set parity holds across the suite**
...
the lightweight `pygx` tier **roughly at parity** with the heavyweight JVM tier across the suite
...
> Geometry-aware `quadbin_geomkring`/`geomkloop`(+explode) are benchmarked separately in [Geometry-aware expansion at scale](#geomk-at-scale).
...
**The `bng_*` functions were benchmarked on a cluster** (DBR 17.3 LTS, spark-path, 1,000
tiles/iteration, 5 measured iterations, both tiers). **Exact cell-set / decoded-geometry parity
holds across the suite** — light output equals heavy output, enforced by the in-cell parity gate and the
`test_parity_bng` cross-tier suite (all parity gates passed; 0 errors).

On *timing*, the pure-Python `pygx` BNG tier runs at **near-parity** with the heavyweight JVM tier:
across the suite the lightweight tier is within **~±20% of heavy (0.82×–1.18×)**, all
sub-millisecond per tile at 1,000 tiles/iteration. roughly a third are light-faster (best `bng_kring` 1.18×);
the rest are marginally heavy-faster (most `bng_cellintersection` 0.82×). These deltas sit within the
...
> Geometry-aware `bng_geomkring`/`geomkloop`(+explode) appear in the table above (cluster parity); their geometry-throughput/scaling numbers are in [Geometry-aware expansion at scale](#geomk-at-scale).
...
**The `custom_*` functions were benchmarked on a cluster** (DBR 17.3 LTS, x86, spark-path,
1,000 tiles/iteration, 5 measured iterations, both tiers). **Exact cell-id/cell-set parity holds across the suite** — light output equals heavy output (cell ids/sets exact; geometry within 1e-6; grid struct
...
parity** with the heavyweight Scala tier across the suite (0.87×–1.34×): it is faster on the cheapest ops
...
> Geometry-aware `custom_geomkring`/`geomkloop`(+explode) are benchmarked separately in [Geometry-aware expansion at scale](#geomk-at-scale).

### Cellfill (`bng_cellfill`, `quadbin_cellfill`, `h3_cellfill`, `custom_cellfill`)

The cellfill functions are grouped aggregators that fill cells whose value is `NULL` by interpolating from their neighbours in the same group — an unweighted mean or inverse-distance weighting over k-ring neighbours — as a `GROUP BY` aggregate over `(cellid, value)` rows. Because the work is cell-space math — no raster tile bytes cross the JVM↔Python boundary — the lightweight tier runs at or near parity with the heavyweight tier.

> **Environment:** Databricks · DBR 18.x · 20 workers · 1,000 cell groups per iteration. `gbx_custom_cellfill` is **lightweight-only** (the heavyweight tier does not have a streamable grid-struct path for this function, so no cross-tier ratio is available).

**Consistency:** All compared cells were **exact or within tolerance (0 divergent)**.

Speedup = heavy ÷ light, ≥ 1× → light faster:

| Function | Heavy (s) | Light (s) | Light speedup |
|---|---|---|---|
| `gbx_quadbin_cellfill` | 2.12 | 1.58 | 1.34× |
| `gbx_bng_cellfill` | 1.70 | 1.65 | 1.03× |
| `gbx_h3_cellfill` | 2.02 | 3.56 | 0.57× |
| `gbx_custom_cellfill` | — (heavy N/A) | 1.95 | — |

The quadbin and BNG functions run at or faster than parity: cell-space geometry is cheap and there are no tile bytes to move across the Python boundary. The H3 gap (0.57×) reflects H3's already-fast C-backed encoder, which leaves less room for the Python advantage. All three comparable pairs are within tolerance or exact.

### Geometry-aware expansion at scale (`geomkring` / `geomkloop`) {#geomk-at-scale}

The geometry-aware expansion functions grow grid rings outward from a *geometry's* covering set (not a single cell). This is a **throughput/scaling** benchmark — how expansion cost grows with the number of input geometries `N` and the ring distance `k` — rather than a light-vs-heavy parity table. Inputs are synthetic **notional building footprints** across five shapes (rectangle, L, U, courtyard-with-hole, irregular) and three sizes (~15 / 50 / 200 m), scattered over a city-scale extent so each building lands on a distinct cell set.

> **Environment:** Databricks **Serverless** (environment 6) · `light_env6` wheel · `geomkring`/`geomkloop`, `boundary-out` · best-of-3 warm timing of a full distributed aggregate (`sum(size(cells))` over all rows). Target cell size ≈ 65 m: **h3** res 10, **quadbin** res 19, **custom** 65 m root cell, **BNG** res 4 (100 m — the nearest discrete BNG step). h3 and quadbin run on WGS84; custom and BNG on EPSG:27700. Footprints come from the `bench/buildings.py` generator. Cell counts are the total emitted across all input geometries. (Serverless wall-clock carries run-to-run variance from autoscaling — read the ordering and magnitudes, not the last digit.)

**Headline** — 10,000 building footprints, h3 res 10, `geomkring` `boundary-out`, `k=10`: **≈ 7.9 s** producing **3,573,051 cells** — about **1,260 buildings/s** and **450k cells/s** on Serverless.

**Across grids** (10,000 buildings, `k=10`, `geomkring`):

| Grid | Resolution (≈65 m cell) | Wall (s) | Cells | Buildings/s | Cells/s |
|---|---|---|---|---|---|
| h3 | res 10 | **7.9** | 3,573,051 | 1,264 | 452k |
| custom | 65 m root (EPSG:27700) | 14.6 | 3,651,174 | 685 | 250k |
| quadbin | res 19 | 41.5 | 4,951,123 | 241 | 119k |
| bng | res 4 (100 m) | 52.3 | 4,762,643 | 191 | 91k |

h3 is fastest — its polyfill uses the h3 library's performant `polygon_to_cells_experimental` (C-backed) and its neighbour step is pure integer index math (`grid_disk`). custom is next (a pure-Python lattice walk). quadbin (web-mercator `(z,x,y)` math + EWKB) and BNG (STRING cell ids + chip geometry) carry more per-cell work; BNG's res 4 also emits coarser 100 m cells. Cell sizes and CRS differ per grid, so read the ordering rather than a direct cell-for-cell comparison.

**Ring distance `k`** (h3, 10,000 buildings; `geomkring` filled disk vs `geomkloop` hollow shell):

| k | `geomkring` (s) | Ring cells | `geomkloop` (s) | Loop cells |
|---|---|---|---|---|
| 1 | 3.03 | 113,181 | 3.02 | 84,430 |
| 5 | 4.25 | 1,050,901 | 4.48 | 324,430 |
| 10 | 7.9 | 3,573,051 | 7.88 | 624,430 |

The filled ring's cell count grows ~`k²` (disk area) while the hollow loop grows ~`k` (perimeter), so at `k=10` the loop emits ~6× fewer cells. Wall-clock tracks the covering-set frontier work (similar for ring and loop) plus a per-building polyfill cost that dominates at small `k` — hence `k=1` and `k=5` are closer together than the cell counts suggest.

**Number of geometries `N`** (h3 res 10, `geomkring` `boundary-out`, `k=10`):

| Buildings (N) | Wall (s) | Cells | Buildings/s |
|---|---|---|---|
| 100 | 0.94 | 35,774 | 106 |
| 1,000 | 2.63 | 357,031 | 380 |
| 10,000 | 7.9 | 3,573,051 | 1,264 |
| 100,000 | 71.5 | 35,733,305 | 1,400 |

Throughput scales ~linearly at volume: **100,000 building footprints expand to 35.7M cells in ≈ 72 s**. Buildings/s climbs with `N` as the fixed job overhead amortizes, plateauing around 1,400 buildings/s.

**Hole modes at scale** — on courtyard footprints (buildings with an interior hole), `hole-in` `k=5` over 1,000 buildings emits 1,796 cells, expanding inward from each hole's edge — the hole-fill traversals work on holed geometries at scale, not just in unit tests.

</TabItem>

</Tabs>

### LiDAR DSM — `rst_binpoints` / `rst_binpoints_agg` / `rst_isoband` / `rst_chm` {#lidar-dsm}

These four functions ship in **both tiers** (lightweight `pyrx` and heavyweight RasterX),
so they take part in the same heavy-vs-light parity comparison as the rest of the raster
family. They belong to the **point-cloud-to-raster** shape family: binning scattered xyz
points onto a pixel grid (`rst_binpoints`/`rst_binpoints_agg`), polygon-extraction per value
band (`rst_isoband`), and DSM/DEM subtraction (`rst_chm`).

> **Shape** (performance.mdx taxonomy): `rst_binpoints*` are geometry-input constructors
> (no tile DataFrame input — similar to `rst_gridfrompoints`); `rst_isoband` is a
> tile-to-feature-array transformer (similar to `rst_polygonize` and `rst_contour`);
> `rst_chm` is a two-tile combiner (similar to `rst_combineavg`).

**Heavy vs light, cluster-measured** (DBR 18 · 512²·4-band·`float32`·EPSG:4326 tile · corpus pool 1,000 tiles).

*Pure-core — one tile, algorithm in isolation:*

| Function | Heavy | Light | Result | Consistency |
|---|---|---|---|---|
| `rst_chm` | 601 ms | 108 ms | **5.6× lightweight** | exact |
| `rst_isoband` | 108 ms | 1,114 ms | **10× heavyweight** | within_tol |
| `rst_binpoints` | 0.04 ms † | 5.9 ms | — | timing-only |

<sub>† Heavy `rst_binpoints` pure-core hit a NoData short-circuit on the corpus tile, so its single-tile time is not a representative binning cost — read its at-scale spark-path row below instead. `rst_binpoints_agg` is an aggregator and has no single-tile pure-core form.</sub>

*Spark-path at scale — amortized per-tile cost (ms) over an N-tile distributed job:*

| Function | Heavy @1,000 | Light @1,000 | @100 (hw / lw) | @10 (hw / lw) |
|---|---|---|---|---|
| `rst_binpoints_agg` | **0.46** | 3.14 | 2.54 / 14.4 | 26.0 / 196 |
| `rst_isoband` | **6.76** | 30.0 | 83.1 / 68.7 | 597 / 456 |
| `rst_chm` | **4.20** | 30.6 | 31.5 / 60.6 | 382 / 404 |
| `rst_binpoints` (light-only ‡) | — | 1.16 | — / 9.39 | — / 136 |

<sub>‡ `rst_binpoints` is a geometry-input constructor (like `rst_gridfrompoints`): the spark-path tile DataFrame carries no geometry column, so only the lightweight tier — which streams points through its own path — has a spark-path form here; the heavyweight tier is pure-core-only for it.</sub>

**How to read these.** `rst_chm` is the textbook pure-core-vs-scale crossover: the lightweight NumPy subtract-and-clip wins **5.6×** on a single tile, but serializing every tile's bytes across the JVM↔Python boundary on each row flips it at scale — the heavyweight tier is **~7.3× faster over a 1,000-tile job**. `rst_isoband` favors the heavyweight tier both in isolation (**10×** — GDAL's C-backed polygon extraction) and at the largest scale (**~4.4×**); the two tiers trade places only at small row counts, where fixed job overhead dominates the per-tile average. `rst_binpoints_agg` favors the heavyweight tier at every scale (**up to ~6.8×**): accumulating one `(x, y, z)` triple per row in the JVM beats collecting a Python array per group. Output stays consistent across tiers where it can be fingerprinted — `rst_chm` **exact**, `rst_isoband` **within tolerance**; `rst_binpoints_agg` returns an aggregate tile that is timing-only in this run. The numbers are one cluster snapshot (absolute timings are environment-dependent); the relative picture and the consistency outcome are the stable takeaways. See [How the benchmark works](#how-the-benchmark-works--how-to-run-it).
