...
**Expected performance characteristics** (local, single-tile, illustrative):

| Function | Notes |
|---|---|
| `rst_binpoints` | NumPy 2D histogram (`np.histogram2d`); dominated by array allocation at low-density clouds, I/O at high density. |
| `rst_binpoints_agg` | Same kernel as `rst_binpoints` — collects ARRAY per group then calls the same histogram path. Aggregator overhead is negligible relative to the binning kernel. |
| `rst_isoband` | `numpy.digitize` + `rasterio.features.shapes` per band. Rasterio `shapes` is C-backed; cost grows with polygon complexity, not raster area. |
| `rst_chm` | `rasterio.warp.reproject` (nearest-neighbour) + NumPy subtraction + clip-to-zero. Cost dominated by the warp (tiles with identical grids take only the subtract + clip). |

No Databricks cluster benchmarks have been run for this family. Add your own timings and
contribute them back as a PR; follow the methodology in the [How the benchmark works](#how-the-benchmark-works--how-to-run-it) section below.
