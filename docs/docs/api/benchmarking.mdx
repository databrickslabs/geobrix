...
**Conclusion:** FILE is not a throughput win for virtual-tile reads — it never wins, in any
configuration tested. The bottleneck is per-tile HTTP stream setup and COG-header parsing
over HTTP — each tile opens a new `FileRef` connection and re-parses the COG header before
reading the window bytes. GDAL windowed reads over FUSE use block-level I/O against the OS
page cache, which reuses each decompressed block across the windows that touch it — FUSE
does **not** stage the whole file.
...
to amortize it. Net: **FILE is a governed file-access mechanism for clusters without a FUSE
mount, not a raster read optimization.**
