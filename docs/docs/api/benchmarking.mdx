...
A dedicated benchmark profile for the large-raster auto-split path (`splitStrategy=auto`) is
planned and will land in a follow-on update. It will measure the split-and-COG-encode
throughput for files that exceed the decoded-memory budget, comparing `serverless` and
`classic` budget modes, and will verify COG-output parity (band values and overview level
count) against the no-split whole-image path. Results will be added to this section once
the profile lands.
:::
