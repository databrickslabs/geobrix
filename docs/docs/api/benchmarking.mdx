...
- **Discrete-grid aggregation (`rst_h3_*`/`rst_quadbin_*`)** is a ~1.7–6× lightweight win and matches within tolerance.
  ...
  > **British National Grid + quadbin raster-grid functions — cluster-measured.** The raster→grid family gained five British National Grid reducers — `rst_bng_rastertogridavg`, `rst_bng_rastertogridcount`, `rst_bng_rastertogridmax`, `rst_bng_rastertogridmin`, `rst_bng_rastertogridmedian` — alongside two more tessellation generators (`rst_quadbin_tessellate`, `rst_bng_tessellate`) and two grid rasterize aggregators (`rst_quadbin_rasterize_agg`, `rst_bng_rasterize_agg`). All nine were benchmarked on the 20-worker cluster with the same light-vs-heavy fingerprint parity check as their `rst_h3_*` / `rst_quadbin_*` siblings (a fixed deterministic cell set for the two aggregators). Because the BNG functions reproject the tile to EPSG:27700 internally and drop pixels outside Great Britain, they are measured on a **British-National-Grid tile** (EPSG:27700 over central London) so they bin **real cells**. On that tile the five BNG reducers agree **within tolerance** across tiers (identical cell sets; per-cell aggregate values agree to rel ≤ 0.001), and `rst_quadbin_tessellate` plus both `*_rasterize_agg` aggregators agree **exactly** (byte-identical cell sets and rasterized output). `rst_bng_tessellate` is the one exception: the heavyweight tier emits a **covering set that also includes the cells touching the tile's exact edge** (48 cells) while the lightweight tier emits only the interior-overlap cells (36 cells, a strict subset — every light cell is a heavy cell). Interior cells match; the difference is confined to the boundary row/column.
  >
  > Representative timings from that run (512² · 4-band · `float32`; the BNG functions on an EPSG:27700 British-National-Grid tile, quadbin on EPSG:4326): pure-core, `rst_bng_tessellate` is a **14.6× lightweight win** (heavy 1.56 s vs light 0.11 s) and `rst_quadbin_tessellate` a **12.9× win** (1.23 s vs 0.10 s, exact). The five BNG reducers, now binning real cells, are **heavyweight-favored** — roughly 0.12–0.15× (heavy ~1.1–1.4 s vs light ~8.9–9.0 s) — because the lightweight tier's per-pixel reprojection-plus-bin over a full raster is dominated by the EPSG:27700 warp and Python-side binning, work the JVM does in-process. The two aggregators, measured on the 1,000-tile spark-path, are heavyweight-favored on a small burned canvas — `rst_bng_rasterize_agg` 8.0 ms vs 12.8 ms/tile (0.62×, exact) and `rst_quadbin_rasterize_agg` 8.3 ms vs 13.2 ms/tile (0.62×, exact). The **BNG reducers and `rst_bng_tessellate` reproject the tile to EPSG:27700 internally**, so their timing includes that warp — unlike the 4326-native H3 and quadbin functions — and a like-for-like reading should account for it. The five BNG reducers and the two tessellation generators are registered on the lightweight tier as table functions (invoked with SQL `LATERAL`), so they appear in the pure-core comparison above rather than as a scalar spark-path column.
