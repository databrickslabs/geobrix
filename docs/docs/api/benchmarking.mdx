...
GeoBrix is [supported on both DBR 17.3 LTS and DBR 18 LTS](../installation#supported-databricks-runtimes). The per-result **Environment** stamps below record the specific cluster each benchmark actually ran on (DBR 17.3 LTS); they are factual records of the run, not a statement that DBR 18 is unsupported.
:::
...
Before any tile is read, the reader must **enumerate the source directory** and open
each file header to determine tile windows. On a UC Volume with thousands of files this
driver-side walk is the dominant planning cost — and it runs every time a job starts,
even when `virtualTiles=true` (the directory walk still happens; only the per-file
`rasterio.open` is skipped by virtual-tile planning).
...
|---|---|---|
| `os.walk` enumeration (default) | 1.357 | — |
| Pre-computed manifest / tile table | **0.037** | **~37× faster** |

The manifest path skips the directory walk and header opens entirely — planning
reduces to a single file or table read regardless of tile count.

:::tip Use a manifest or tile table for large directories
When a directory contains thousands of rasters, supply a pre-computed tile list
instead of loading the directory directly:
...
Both options reduce planning from an `os.walk` over the Volume FUSE mount (1.4 s at
10K files, growing linearly) to a single file or table read (**~37× faster** at that
scale). The benefit is purely at plan time and is independent of which functions run
on the tiles.
:::
...
The tables above compare tiers; this one isolates the cost of the **input tile itself** on the lightweight tier. It runs the same 1,000-tile spark-path job twice — once with a **materialized** input tile (pixel bytes carried in the row) and once with a **virtual** one (a `path` + `window`, no bytes) — for every general raster function. `Virtual speedup = materialized ÷ virtual`, so **above 1× means the virtual tile is faster**. `Disposition` records what the function did with that virtual tile: `materialized` (it read pixels), `deferred` (it answered from the header / georeferencing without touching pixels), or `na` (it takes no single input tile — the grouped aggregators and the array / byte constructors).

> **Environment:** Databricks · DBR 18.x · 20 workers · 512² 4-band `float32` tiles · 1,000 tiles per iteration. Both columns are lightweight-tier whole-job wall-clock amortized over the tiles, measured in the same run so the pair is directly comparable.
...
**The takeaway:** virtual tiles are a lazy-load win precisely for the operations that never touch pixels — metadata reads and the set-CRS / set-SRID / nodata / band-select edits that defer until a later terminal step. A pipeline that chains several of those deferred edits before one materializing step pays the pixel I/O **once, at the end**, instead of at every step. For pixel-generating or pixel-reading work the raster must be read regardless, so a materialized input is marginally cheaper there. (`rst_rastertoworldcoord` is the one `deferred` row that doesn't win: it derives a full per-pixel coordinate grid from the georeferencing, so it is compute-bound, not I/O-bound.)

#### FILE vs FUSE: large-COG multi-window results {#file-capability-cog-multiwindow}

> **Environment:** dogfood DBR-19.5 snapshot · `fileReferenceCreationMode=MANAGED` ·
> i3.xlarge · `file_supported()=True` · 1 measured iteration, `warmup=0`.
...
:::note Per-tile-open (non-amortized) regime
The table above measures the **non-amortized regime**: `warmup=0`, each tile opens a new
`FileRef` connection and re-parses the COG header before reading its window. In this regime
FUSE wins because the per-open HTTP cost is paid once per tile with no reuse.

With **per-partition open-amortization** — where the source is opened once per partition
and all windows from that source are read from the same cached dataset handle — the byte-range
stream wins **10–290×** over FUSE for windowed COG reads. See
[Virtual tile read performance](../api/performance#virtual-tile-read-performance) for the
amortized numbers and guidance on enabling amortization via the grouped executor.
:::

**Conclusion (non-amortized, per-tile-open):** In the per-tile-open regime, FILE is not a
throughput win — it never wins. The bottleneck is per-tile HTTP stream setup and COG-header
parsing over HTTP — each tile opens a new `FileRef` connection and re-parses the COG header
before reading the window bytes. GDAL windowed reads over FUSE use block-level I/O against
the OS page cache, which reuses each decompressed block across the windows that touch it —
FUSE does **not** stage the whole file.

The **margin** depends on the COG's internal structure. FUSE wins by ~2× on 256px-tiled
COGs (where a 256px window maps to exactly one block), but the advantage **shrinks toward a
wash on real production COGs**: when the internal block is larger than the read window
(512px for NAIP, 1024px for Sentinel-2), *both* FILE and FUSE must fetch and decompress the
whole block, so FILE's byte-range selectivity is neutralized and only the per-tile HTTP
overhead separates them. The one regime where FILE loses badly (11×) is **low reuse** — many
distinct files read once each — because the per-open HTTP cost dominates with no block reuse
to amortize it. Net: **in the non-amortized regime, FILE is a governed file-access mechanism
for clusters without a FUSE mount, not a raster read optimization.**

The standard bench corpus uses many small whole-file tiles. To reproduce these results,
use the COG multi-window corpus: K large Cloud-Optimized GeoTIFFs, each referenced by M
narrow window rows.
...
Use `warmup=0, measured=1` for spark-path legs — one full iteration already averages
over all windows, so extra measured passes add noise without improving the estimate.

```python
from databricks.labs.gbx.bench.readers import run_virtual_tile_pixel_read

results_on  = run_virtual_tile_pixel_read(spark, path="<corpus_dir>",
                manifest="<manifest_path>",
                run_id="cog-file-on",  disable_file=False, where="cluster", warmup=0, measured=1)
results_off = run_virtual_tile_pixel_read(spark, path="<corpus_dir>",
                manifest="<manifest_path>",
                run_id="cog-file-off", disable_file=True,  where="cluster", warmup=0, measured=1)
```
