...
Each mode runs **twice** — once writing one `.nc` file per row/partition (the default
"parts" shape) and once with `singleFile=true`, which merges the fragments into a single
`.nc` on the driver. The two shapes are recorded as distinct rows (the singleFile leg is
tagged `[singleFile]` in its note and written to a separate output directory) so a run
carries a real parts-vs-single throughput comparison.
