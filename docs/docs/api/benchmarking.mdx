...
- **Discrete-grid aggregation (`rst_h3_*`/`rst_quadbin_*`)** is a ~1.7–6× lightweight win and matches within tolerance.
  ...
  > **New grid functions now in the bench set (numbers pending the first cluster run).** The raster→grid family gained five British National Grid reducers — `rst_bng_rastertogridavg`, `rst_bng_rastertogridcount`, `rst_bng_rastertogridmax`, `rst_bng_rastertogridmin`, `rst_bng_rastertogridmedian` — alongside two more tessellation generators (`rst_quadbin_tessellate`, `rst_bng_tessellate`) and two grid rasterize aggregators (`rst_quadbin_rasterize_agg`, `rst_bng_rasterize_agg`). All nine are registered in the benchmark harness and run through the same 20-worker cluster configuration and row ladder as their `rst_h3_*` / `rst_quadbin_*` siblings, with the same light-vs-heavy fingerprint parity check (a fixed deterministic cell set for the two aggregators). Their measured rows will be backfilled into the tables below after the first cluster run. Note that the BNG reducers and `rst_bng_tessellate` reproject the tile to EPSG:27700 internally, so their timing includes that warp — unlike the 4326-native H3 and quadbin functions — and a like-for-like reading should account for it.
