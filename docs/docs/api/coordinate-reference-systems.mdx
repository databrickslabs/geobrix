...
- **Z coordinates.** A geometry whose vertices all carry a finite Z keeps its Z; a 2D geometry stays 2D. Where only *some* vertices carry a Z, the current behavior is that `st_transformcrs` reprojects the geometry as 2D — reprojecting a missing Z would propagate it into X and Y and destroy the horizontal position — while `st_setcrs` keeps the partial Z, because stamping an SRID never moves coordinates. A missing Z is never filled in with a substitute value. If you chain the **Python/Scala core** functions, chain them in **binary**: a WKT round trip between calls writes a missing elevation as a token the reader then reads as 2D, so Z can be lost. SQL is unaffected — it always returns `BINARY`. See [the caveat in the function reference](./vectorx-functions#crs).
