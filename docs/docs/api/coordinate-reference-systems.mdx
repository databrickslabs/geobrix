...
# Call once, before running any CRS transform:
...
`register_proj_grids` must be called **before** any transform that needs the grid. Transforms defined and run after the call see the grids on every worker — including workers that scale up later, because the directory list is embedded in the UDF closures (lightweight tier) and in each query plan (heavyweight tier). Transforms already planned before the call do not pick up new directories retroactively.
