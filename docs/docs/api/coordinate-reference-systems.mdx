...
Call `register_proj_grids` once at the start of your session, before running transforms. Every transform after that — including on workers that join later in the session — finds the registered grids. Transforms already defined before the call are not updated retroactively.
