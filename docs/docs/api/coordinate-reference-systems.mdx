...
:::note VectorX and GridX CRS surfaces
The **VectorX** CRS family (`st_crs` / `st_setcrs` / `st_transformcrs`) and the broader **GridX** CRS surface (custom-CRS input reprojection for `polyfill` / `tessellate` / `pointascell`, grid CRS accessors) are separate follow-on workstreams; their rows will be added here when those functions ship.
:::
