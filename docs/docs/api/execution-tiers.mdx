...
|---|---|---|
| `gbx_rst_bng_rastertogrid{avg,count,max,min,median,sum}` | Both | Light support via `pyrx` (`pygx._bng` cell math + `rasterio`, EPSG:27700 auto-warp, STRING cell IDs) |
| `gbx_rst_quadbin_rastertogrid{avg,count,max,min,median,sum}` | Both | Light support available via `pyrx` |
