...
The one-line swap keeps your code identical across tiers. The lightweight tier is functionally complete for raster — it implements every `rst_*` function — and for VectorX, implementing every `gbx_st_*` function (MVT, TIN, legacy migration).
