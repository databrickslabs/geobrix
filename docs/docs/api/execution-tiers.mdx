...
| **Reference / passthrough tile ops** (`rst_clip`, `rst_setsrid`, **identity `rst_transform`** where target CRS == source CRS) | Reference/passthrough class — produce no new pixels; reference existing backing data | These ops belong to the no-new-pixels class: their output describes a region of the original backing store rather than computing fresh pixel values. `virtualize_dir` has no meaningful effect on them — there are no newly-produced pixels to externalize |
