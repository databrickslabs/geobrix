...
within the Python worker. The heavyweight tier consumes only **materialized** tiles (raster bytes
present) — the JVM cannot lazily read from a Unity Catalog Volume path the way a Python worker can,
so a bytes-free virtual tile is not usable by heavy functions without first converting it to bytes.
