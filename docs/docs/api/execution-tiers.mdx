...
| Function coverage | Full raster set | Full raster set — every `rst_*` function except BNG/quadbin raster-grid functions (heavy-tier for now; lightweight support planned) |
...
The one-line swap keeps your code identical across tiers. The lightweight tier is functionally complete for raster — it implements every `rst_*` function except the BNG/quadbin raster-grid functions listed below (heavy-tier for now; lightweight support planned) — and for VectorX, implementing every `gbx_st_*` function (MVT, TIN, legacy migration).

**Raster-grid surface (BNG/quadbin) — current tier status:**

| Function family | Tier | Notes |
|---|---|---|
| `gbx_rst_bng_rastertogrid{avg,count,max,min,median}` | Heavy only | Light quadbin planned; light BNG pending `pygx` BNG raster integration |
| `gbx_rst_quadbin_rastertogrid{avg,count,max,min,median}` | Both | Light support available via `pyrx` |
| `gbx_rst_quadbin_tessellate`, `gbx_rst_bng_tessellate` | Heavy only | Light quadbin planned; light BNG pending `pygx` BNG raster integration |
| `gbx_rst_quadbin_rasterize_agg`, `gbx_rst_bng_rasterize_agg` | Heavy only | Light quadbin planned; light BNG pending `pygx` BNG raster integration |
