...
The **lightweight tier** introduces a bytes-free tile representation called a **virtual tile** — a
reference to a source path and a pixel window, rather than a self-contained in-memory raster payload.
Virtual tiles are a lightweight-tier concept: they are created, consumed, and manipulated entirely
within the Python worker. The heavyweight tier does not support them — passing a virtual tile to a
heavyweight function raises an error, because the JVM cannot lazily read from a Unity Catalog Volume
path the way a Python worker can.

**If you get an error passing a tile to a heavyweight function**, the tile is likely virtual. You have
two options:
...
| **Deferrable tile ops** (`rst_clip`, `rst_setsrid`, transform-intent) | Stay virtual when input is virtual | Chain them for free — no pixels touched |
| **Pixel-producing tile ops** (slope, aspect, hillshade, terrain, focal, mapalgebra, spectral indices, rasterize, resample, …) | Materialize the window and return bytes | Pass `virtualize_dir` to write the computed result to a durable path and return a light virtual row instead |
...
| **Crossing to heavy** | Error if tile is virtual | Heavy raises an error on a virtual tile — stay in light (recommended), or materialize first (`materialize=True`, or write + read back) before handing off |
