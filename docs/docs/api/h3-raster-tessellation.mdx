...
Each row in the result is a v2 tile struct — `cellid` (the H3 cell integer ID), `raster` (clipped raster bytes), `path`, `window`, `clip_polygon`, `clip_crs`, `crs`, and `metadata`.
