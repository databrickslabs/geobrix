...
**QuadBin and BNG grid variants** (both tiers — heavyweight Scala/JVM TypedImperativeAggregates and lightweight `pyrx` aggregators backed by `pygx._quadbin` / `_bng` cell math + `rasterio`; the BNG family auto-warps to EPSG:27700 — listed here because they share the same reduce-to-one-raster shape):
