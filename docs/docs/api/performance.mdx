...
| Mode | Per-open cost | Per-window cost (amortized) | Best for |
|---|---|---|---|
| **Byte-range stream** (`.open()`) | Scales with file size and layout | ~2 ms (COG) | Grouped reads: one source, many windows |
| **FUSE** (`.as_local_file()`) | Flat ~7 ms at any size | ~124 ms (cold random window) | Per-tile-open when amortization is not possible |
...
:::warning Large source rasters
The cost to *open* a large file via byte-range scales with its size and internal layout. For a Cloud-Optimized GeoTIFF it grows from milliseconds (small) to seconds (multi-GB). For a **large striped GeoTIFF**, parsing the strip-offset table over byte-range can reach **47–54 seconds per open** at 10 GB. Never open a large striped source once per tile via byte-range — either amortize (open once, read all windows) or use FUSE for per-tile access. **Prefer COGs** for windowed access at scale: COG open costs are far lower and windowed reads avoid unnecessary byte fetching.
:::
