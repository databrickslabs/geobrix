...
**Heavy-tier variants** (Scala/JVM CollectionGenerators, not Python UDTFs — listed here because they share the same fan-out shape):
...
**Heavy-tier variants** (Scala/JVM TypedImperativeAggregates, not Arrow pandas_udf aggregators — listed here because they share the same reduce-to-one-raster shape):
