...
:::note Lightweight tier (pyrx)
The lightweight implementation is backed by `pygx._bng` cell math and `rasterio`. It auto-warps the input to **EPSG:27700** via `rasterio.warp` before rasterizing, parses the **STRING** BNG cell IDs, and burns each cell's geometry/value into the output band — matching the heavyweight cell set and burn values exactly.
:::
