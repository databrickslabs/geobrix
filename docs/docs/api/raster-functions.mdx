...
:::note Write `calc` without spaces
On the heavy tier the `calc` expression must contain **no spaces** (`(A-B)/(A+B)`, not `(A - B) / (A + B)`) — the `gdal_calc` invocation is whitespace-delimited. The lightweight (NumExpr) tier accepts either, so a space-free `calc` is the portable form across both tiers.
:::
