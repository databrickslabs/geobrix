...
Per-pixel count of valid (non-NoData) inputs across an `ARRAY` of aligned tiles. Use alongside the other `rst_combine*` reducers to build multi-statistic temporal or ensemble composites.
...
Per-pixel maximum across an `ARRAY` of aligned tiles. Use to build cloud-free maximum-NDVI composites or storm-peak accumulations.
...
Per-pixel median across an `ARRAY` of aligned tiles. Robust to outliers — useful for seasonal median composites from time-series imagery.
...
Per-pixel minimum across an `ARRAY` of aligned tiles. Use to extract the lowest-cloud-probability pixel from a time stack or to track cumulative minimum temperatures.
...
Per-pixel population standard deviation across an `ARRAY` of aligned tiles. Quantifies variability in a time-series or multi-scene ensemble — high values indicate heterogeneous pixels.
...
Per-pixel sum across an `ARRAY` of aligned tiles. Use to accumulate total precipitation from a daily time-series or to sum spectral flux across dates.
