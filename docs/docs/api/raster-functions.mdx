...
:::note Lightweight-only `out_crs` parameter (no-op for BNG)
...
:::note Memory & scale — thin the points that feed a TIN

A Delaunay TIN must hold **all** of a group's points in memory to triangulate, so peak memory is
**proportional to the points per group** — and, unlike [`rst_binpoints_agg`](#rst_binpoints_agg),
a triangulation is inherently global and **cannot** be made grid-bounded the way
[`bin_points_tiled`](#rst_binpoints_agg) bounds binning. At full LiDAR density a single tile's ground
returns (hundreds of thousands to millions of points) can exhaust worker RAM.

A bare-earth TIN is near-identical from a **representative subset**, so thin the input points before
the aggregate — `df.sample(fraction=…)` down to a bounded per-group count (e.g. ~150 k), or use
smaller tiles. This is standard LiDAR TIN practice and keeps memory bounded regardless of source
density. See the [LiDAR reader](../readers/lidar) and [Performance](./performance).
...
:::note Lightweight-only `out_crs` parameter
...
:::note Lightweight tier (pyrx)
Powered by **rasterio** + **pygx** custom-grid cell math. Aggregate — `groupBy(...).agg(rx.rst_custom_rasterize_agg("cellid", "value", ...))` burns each custom-grid cell's centroid pixel (or spatial-envelope pixels with `mode='spatial_envelope'`) into one raster tile per group. Cell IDs are BIGINT encoded by the custom-grid layout. When `value` is omitted or null, all burned pixels carry `1.0` (presence mask). The extent auto-derives from the cell set when canvas parameters are null.
:::

:::warning Lightweight SQL returns BINARY (not the tile struct)
Heavyweight `gbx_rst_custom_rasterize_agg` returns a tile **`STRUCT<cellid, raster, path, window, clip_polygon, clip_crs, crs, metadata>`** (the v2 8-field tile); the lightweight **SQL** function returns **`BINARY`** (the raster bytes). A PySpark grouped-aggregate `pandas_udf` cannot return a `StructType`, so the lightweight SQL aggregate returns the raster payload as BINARY. The lightweight **Python** wrapper `rx.rst_custom_rasterize_agg(...)` returns the full tile struct (it composes the aggregate with a tile-wrapping step), so only raw SQL differs. To rebuild the tile-struct equivalent in SQL, select the group key as `cellid` and wrap the BINARY with `gbx_rst_fromcontent`:
...
SELECT
  ...
  gbx_rst_fromcontent(
    gbx_rst_custom_rasterize_agg(
      ...
      cast(null as int), cast(null as int), 'centroids', cast(1 as int)
    ),
    'GTiff'
  ) AS tile
...
Streaming aggregator that burns custom-grid cell values into one raster tile per group. Cell IDs are BIGINT encoded by the `gbx_custom_grid(...)` layout. This is the **inverse** of `rst_custom_rastertogrid*`: where those functions reduce raster pixels to per-cell statistics, `rst_custom_rasterize_agg` reconstructs a raster from per-cell values. Pass the custom-grid struct (from `gbx_custom_grid(...)`) as the third argument; the canvas extent auto-derives from the cell set when all spatial parameters are null.

**Signature:** `rst_custom_rasterize_agg(cellid: Column, value: Column, grid: Column, out_srid: Column, pixel_size: Column, xmin: Column, ymin: Column, xmax: Column, ymax: Column, width: Column, height: Column, mode: Column, kring_pad: Column): Column`

**Parameters:** `cellid` — custom-grid cell ID (BIGINT); `value` — numeric burn value (null → `1.0` presence mask); `grid` — custom-grid struct from `gbx_custom_grid(...)`; `out_srid` — EPSG code for the output CRS; `pixel_size` — output raster cell size in target CRS units (null → auto-derived from cell dimensions); `xmin/ymin/xmax/ymax` — output extent (null → auto-derived from the cell set + `kring_pad`); `width/height` — output dimensions in pixels (null → auto-derived); `mode` — `"centroids"` (default) or `"spatial_envelope"`; `kring_pad` — neighbourhood expansion in rings for auto-extent (default 1)

<FunctionExamples
  name="rst_custom_rasterize_agg"
  sql={rasterxSqlCode}                  sqlSource="docs/tests/python/api/rasterx_functions_sql.py"
  pythonLight={rasterxAggregatorsLightCode} pythonLightSource="docs/tests/python/api/rasterx_aggregators_python_light.py"
  pythonHeavy={rasterxCode}             pythonHeavySource="docs/tests/python/api/rasterx_functions.py"
  scala={scalaApiExamplesCode}          scalaSource="docs/tests/scala/api/ScalaApiExamples.scala"
...
:::note Lightweight Python — virtual-tile force-output
The lightweight **Python** binding accepts three optional keyword arguments the SQL and heavyweight tiers do not — `virtualize_dir`, `virtualize_prefix`, and `materialize` — controlling whether the produced tile carries raster bytes or a bytes-free virtual reference. See [Virtual-tile force-output params](#virtual-tile-overrides).
:::
...
:::caution Serverless memory — you own the bytes
...
:::note Tier differences — virtual vs materialized
...
:::caution `materialize=True` errors over the Serverless cap
...
`rst_fromfile` pulls columnar raster paths into a tile column inline. To ingest rasters as a normal Spark job — partitioned parallel reads, optional tiling (`sizeInMB`), FUSE-safe Volume staging — use the **[Raster Reader](../readers/raster)** (`raster_gbx` / `gtiff_gbx`, or heavyweight `gdal` / `gtiff_gdal`): `spark.read.format("raster_gbx").load(path)`. It, too, emits virtual tiles by default.
:::

:::note Have the bytes already, or need a materialized tile? Use `rst_fromcontent`
...
See [interpolation & fill](./grid-raster-tessellation#interpolation-and-fill) to choose among sampling, resampling, and gap-filling.
...
## LiDAR / Point-Cloud DSM {#lidar-dsm}

Four functions for building raster products from LiDAR point clouds. Available on **both tiers** (`pyrx` lightweight and `rasterx` heavyweight). Load point clouds with the [`lidar_gbx` reader](../readers/lidar). For an end-to-end walkthrough — `.laz` → DSM/DTM binning → Canopy Height Model — see the [Wireless Coverage example](../notebooks/wireless-coverage).

### rst_binpoints {#rst_binpoints}

<Tier both/>

Bin a **per-row `ARRAY<DOUBLE>`** of (x, y, z) coordinate triples into a single-band
Float32 raster tile. Each pixel carries the reduction of all points whose centroid falls
within the pixel cell. Empty cells carry NoData (−9999). The bounding box uses a
half-open interval on the upper x/y edges to prevent double-counting when DSM tiles
are assembled from adjacent strips.

**What it computes.** This is the point-cloud→raster (DSM-building) step: every input
point is dropped into the grid cell its (x, y) falls in, and the pixel value is the chosen
reduction of the z-values that landed there. With the default `"max"` that is the *tallest*
return per cell — a surface model (DSM), exactly the input [`rst_chm`](#rst_chm) differences
against a DEM; `"min"`/`"mean"`/`"median"`/`"count"`/`"percentile:<p>"` reduce the same
points into other surfaces.

**Signature:** `rst_binpoints(x_array, y_array, z_array, xmin, ymin, xmax, ymax, width_px, height_px, srid, [statistic])`

**Parameters:** `x_array`, `y_array`, `z_array` — parallel `ARRAY<DOUBLE>` columns of point coordinates and z-values; `xmin`/`ymin`/`xmax`/`ymax` — extent in CRS units; `width_px`/`height_px` — output pixel dimensions; `srid` — EPSG code; `statistic` — one of `"max"` (default, highest return for DSMs), `"min"`, `"mean"`, `"median"`, `"count"`, or `"percentile:<p>"` (e.g. `"percentile:90"`).
...
### rst_binpoints_agg {#rst_binpoints_agg}

<Tier both/> <Impl groupedAgg/>

Aggregator variant of [`rst_binpoints`](#rst_binpoints): stream **one scalar (x, y, z) point
per row** into a DSM tile via `groupBy(...).agg(rx.rst_binpoints_agg(...))`. The
`xmin`/`ymin`/`xmax`/`ymax`/`width_px`/`height_px`/`srid`/`statistic` arguments are
per-group constants. The per-cell binning is identical to [`rst_binpoints`](#rst_binpoints) —
see **What it computes** there for the reduction semantics and statistic modes — it just
streams one point per row instead of a per-row array.

The lightweight SQL aggregate returns **`BINARY`** (raster bytes). Wrap with
`gbx_rst_fromcontent(<agg>, 'GTiff')` to rebuild a tile struct. The lightweight Python
wrapper `rx.rst_binpoints_agg(...)` returns the full tile struct.

**Signature:** `rst_binpoints_agg(x, y, z, xmin, ymin, xmax, ymax, width_px, height_px, srid, [statistic])`

**Parameters:** `x`, `y` — scalar `DOUBLE` columns of point coordinates; `z` — scalar `DOUBLE` z-value; remaining parameters match [`rst_binpoints`](#rst_binpoints).

<FunctionExamples
  name="rst_binpoints_agg"
  sql={rasterxSqlCode}                     sqlSource="docs/tests/python/api/rasterx_functions_sql.py"
  pythonLight={rasterxAggregatorsLightCode} pythonLightSource="docs/tests/python/api/rasterx_aggregators_python_light.py"
  pythonHeavy={rasterxCode}                pythonHeavySource="docs/tests/python/api/rasterx_functions.py"
  scala={scalaApiExamplesCode}             scalaSource="docs/tests/scala/api/ScalaApiExamples.scala"
...
:::note Memory & scale — binning full-density LiDAR

Both `rst_binpoints_agg` (grouped — realizes a whole group's points in the worker) and
`rst_binpoints` (scalar — realizes a row's point arrays) hold memory **proportional to the
points per group**. That is fine at moderate density, but a full-resolution tile (millions of
returns — e.g. a 1 km tile of 1 m LiDAR) can exhaust worker RAM.

For large or full-density point clouds, use the bounded light-tier helper
**`rx.bin_points_tiled(df, x, y, z, by=[…], xmin=…, ymin=…, xmax=…, ymax=…, width, height, srid, stat)`**.
It runs a **two-stage native aggregation** — per-point pixel index → native `groupBy(tile, cell)`
reduction → placement — so peak memory is the **raster grid (W×H)**, independent of point count.
It returns the same tile struct (drops straight into `rst_chm`), and its output is **identical** to
`groupBy(by).agg(rst_binpoints_agg(…))` for `max`/`min`/`mean`/`count`. `median`/`percentile` (which
need every value in a cell) delegate to the buffered aggregator — decimate or use smaller tiles for
those. Prefer `bin_points_tiled` over a `collect_list(all points) → rst_binpoints` pattern, which
buffers the whole cloud. See [Performance](./performance) and the [LiDAR reader](../readers/lidar).

:::

---

### rst_isoband {#rst_isoband}

<Tier both/>

Reclassify a raster band into value bins defined by a `breaks` array and return one
polygon per **contiguous patch** of pixels in the same bin. Useful for producing
height-band polygons from a CHM or DSM, or elevation isobands from a DEM.

Returns `ARRAY<STRUCT<geom_wkb BINARY, band INT, lower DOUBLE, upper DOUBLE>>` — one
entry per contiguous region. `geom_wkb` is a WKB Polygon in the raster's own CRS.
NoData pixels are excluded. Pixels below `breaks[0]` or `>= breaks[-1]` are dropped.

Each bin is **half-open — `[breaks[i], breaks[i+1])`** — closed on the lower bound and
open on the upper, so a value equal to a break belongs to the band that break *opens*.
For example, with `breaks = array(0, 50, 100, 150, 200)`, a value of `55.0` — and `50.0`
itself — falls in the `[50.0, 100.0)` band (`band` index 1), while `100.0` moves up to
the `[100.0, 150.0)` band and `200.0` (equal to `breaks[-1]`) is dropped.

Implemented with `numpy.digitize` + `rasterio.features.shapes` — no GDAL required.

**Signature:** `rst_isoband(tile, breaks)`

**Parameters:** `tile` — tile struct column; `breaks` — `ARRAY<DOUBLE>` of N+1 strictly-ascending boundary values defining N bands.
...
Compute a **Canopy Height Model (CHM)** from a Digital Surface Model (DSM) tile and a
Digital Elevation Model (DEM) tile: `CHM = clamp(align(DSM → DEM grid) − DEM, min=0)`.

**What it computes.** The DSM holds the height of the *top* surface in each cell (canopy,
rooftops) and the DEM holds bare-earth ground elevation, so subtracting one from the other
leaves the height of whatever stands above the ground — canopy or object height — in the
DEM's units. It is a per-cell **subtraction**, not an aggregation: taking the max z per cell
is how you build the DSM upstream (e.g. `rst_binpoints` over LiDAR returns), and `rst_chm`
then differences that surface against the DEM.

The DSM is warped onto the DEM's grid (CRS, extent, width, height) via
nearest-neighbour resampling before subtraction. Negative differences (where the DSM
dips below the DEM due to noise) are clamped to 0. NoData in either input propagates
to the output. Output is a Float32 GeoTIFF tile with NoData −9999.

**Signature:** `rst_chm(dsm_tile, dem_tile)`

**Parameters:** `dsm_tile` — Digital Surface Model tile (defines the surface heights); `dem_tile` — Digital Elevation Model tile (defines the bare-earth reference; also defines the output grid).
...
**Signature:** `rst_bng_tessellate(tile: Column, resolution: Column, [assignment: Column], [coverage: Column]): Column` — assignment is `"centroid"` (default) or `"covering"`; coverage is `"complete"` (default) or `"sparse"`.
...
**Signature:** `rst_h3_tessellate(tile: Column, resolution: Column, [assignment: Column], [coverage: Column]): Column` — Tessellate raster to H3 cells. assignment is `"centroid"` (default — pixel-centroid single-assignment) or `"covering"` (every overlapping cell, clipped); coverage is `"complete"` (default) or `"sparse"`. See [Grid raster tessellation](./grid-raster-tessellation) for the full guide.
...
**Signature:** `rst_quadbin_tessellate(tile: Column, resolution: Column, [assignment: Column], [coverage: Column]): Column` — assignment is `"centroid"` (default) or `"covering"`; coverage is `"complete"` (default) or `"sparse"`.
...
:::note Lightweight tier (pyrx)
The lightweight implementation is backed by **pygx** custom-grid cell math and **rasterio**. It enumerates the overlapping custom-grid cells for the raster and clips one tile per cell, emitting Long-keyed chips (custom cell IDs as BIGINT). The raster is expected to be in the custom grid's native CRS — no automatic reprojection is performed; reproject upstream with `rst_transform` if needed.
:::

Tessellate a raster to user-defined custom-grid cells — one row per overlapping cell, each clipped to that cell's extent. Pass a custom-grid struct (from `gbx_custom_grid(...)`) and a resolution integer that selects the cell size level. Cell IDs are **BIGINT** encoded by the custom-grid layout. For the covering-mode overlap rule (positive-area overlap, boundary touches excluded), see [`rst_h3_tessellate`](#rst_h3_tessellate).

**Signature:** `rst_custom_tessellate(tile: Column, grid: Column, resolution: Column, [assignment: Column], [coverage: Column]): Column` — assignment is `"centroid"` (default) or `"covering"`; coverage is `"complete"` (default) or `"sparse"`.

**SQL:**

<FunctionExamples
  name="rst_custom_tessellate"
  sql={rasterxSqlCode}                  sqlSource="docs/tests/python/api/rasterx_functions_sql.py"
  pythonLight={rasterxTransformsLightCode} pythonLightSource="docs/tests/python/api/rasterx_transforms_python_light.py"
  pythonHeavy={rasterxCode}             pythonHeavySource="docs/tests/python/api/rasterx_functions.py"
  scala={scalaApiExamplesCode}          scalaSource="docs/tests/scala/api/ScalaApiExamples.scala"
...
See [Grid raster tessellation](./grid-raster-tessellation) for the coverage / assignment / fill model shared by all grids.
...
:::note Tier differences — SQL invocation
...
**Signature:** `rst_h3_rastertogridavg(tile: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column`
...
**Signature:** `rst_h3_rastertogridcount(tile: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column` — Pixel count (Double) per H3 cell.
...
**Signature:** `rst_h3_rastertogridmax(tile: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column` — Max value per H3 cell.
...
**Signature:** `rst_h3_rastertogridmedian(tile: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column` — Median value per H3 cell.
...
**Signature:** `rst_h3_rastertogridmin(tile: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column` — Min value per H3 cell.
...
**Signature:** `rst_h3_rastertogridstddev(tile: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column` — Population standard deviation of pixel values per H3 cell.
...
**Signature:** `rst_h3_rastertogridsum(tile: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column` — Sum of pixel values per H3 cell.
...
**Signature:** `rst_h3_rastertogridvariance(tile: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column` — Population variance of pixel values per H3 cell.
...
**Signature:** `rst_quadbin_rastertogridavg(tile: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column` — Mean pixel value per quadbin cell.
...
**Signature:** `rst_quadbin_rastertogridcount(tile: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column` — Pixel count (Double) per quadbin cell.
...
**Signature:** `rst_quadbin_rastertogridmax(tile: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column` — Max pixel value per quadbin cell.
...
**Signature:** `rst_quadbin_rastertogridmedian(tile: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column` — Median pixel value per quadbin cell.
...
**Signature:** `rst_quadbin_rastertogridmin(tile: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column` — Min pixel value per quadbin cell.
...
**Signature:** `rst_quadbin_rastertogridstddev(tile: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column` — Population standard deviation of pixel values per quadbin cell.
...
**Signature:** `rst_quadbin_rastertogridsum(tile: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column` — Sum of pixel values per quadbin cell.
...
**Signature:** `rst_quadbin_rastertogridvariance(tile: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column` — Population variance of pixel values per quadbin cell.
...
**Signature:** `rst_bng_rastertogridavg(tile: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column` — Mean pixel value per BNG cell.
...
**Signature:** `rst_bng_rastertogridcount(tile: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column` — Pixel count (Double) per BNG cell.
...
**Signature:** `rst_bng_rastertogridmax(tile: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column` — Max pixel value per BNG cell.
...
**Signature:** `rst_bng_rastertogridmedian(tile: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column` — Median pixel value per BNG cell.
...
**Signature:** `rst_bng_rastertogridmin(tile: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column` — Min pixel value per BNG cell.
...
**Signature:** `rst_bng_rastertogridstddev(tile: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column` — Population standard deviation of pixel values per BNG cell.
...
**Signature:** `rst_bng_rastertogridsum(tile: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column` — Sum of pixel values per BNG cell.
...
**Signature:** `rst_bng_rastertogridvariance(tile: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column` — Population variance of pixel values per BNG cell.
...
## Grid Functions (Custom Grid) {#custom-grid}

See [Grid raster tessellation](./grid-raster-tessellation) for the coverage / assignment / fill model shared by all grids.

Aggregate raster values to user-defined custom-grid cells (8 total). Custom grids are defined via `gbx_custom_grid(origin_x, origin_y, min_x, min_y, max_x, max_y, root_cell_size_x, root_cell_size_y, crs_srid)` — a regular planar grid with an explicit origin, bounds, and cell size. Cell IDs are **BIGINT** encoded by the custom-grid layout, and the resolution parameter selects the subdivision level. The raster is expected to be in the grid's native CRS — no automatic reprojection is performed; use `rst_transform` upstream if needed.

:::note Lightweight tier (pyrx)
The custom raster→grid reducers are available in both tiers. The lightweight implementation is backed by `pygx` custom-grid cell math and `rasterio`: it samples pixels per cell and emits BIGINT cell IDs — matching the heavyweight cell set and measures exactly.
:::

:::warning NoData and NULL
A cell that covers only NoData pixels returns **`NULL`** for the measure (not `0` or `NaN`) — the same behavior as the H3, quadbin, and BNG reducers. `NULL` is safe to filter (`WHERE measure IS NULL`) and is excluded by SQL aggregates like `MAX`/`MIN`/`AVG`.
:::
...
Mean pixel value per custom-grid cell.

:::note Tier differences — SQL invocation
Heavyweight registers `gbx_rst_custom_rastertogridavg` as a scalar (ARRAY-returning) function — call it directly and `explode(...)` to flatten to rows. The lightweight (pyrx) tier registers it as a streaming **table function**, so lightweight SQL must use `LATERAL`. Both forms appear in the SQL tab below.
:::

**Signature:** `rst_custom_rastertogridavg(tile: Column, grid: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column` — Mean pixel value per custom-grid cell.

**SQL:**

<FunctionExamples
  name="rst_custom_rastertogridavg"
  sql={rasterxSqlCode}                  sqlSource="docs/tests/python/api/rasterx_functions_sql.py"
  pythonLight={rasterxGridaggLightCode} pythonLightSource="docs/tests/python/api/rasterx_gridagg_python_light.py"
  pythonHeavy={rasterxCode}             pythonHeavySource="docs/tests/python/api/rasterx_functions.py"
  scala={scalaApiExamplesCode}          scalaSource="docs/tests/scala/api/ScalaApiExamples.scala"
...
Valid (non-NoData) pixel count per custom-grid cell (measure is DOUBLE).

:::note Tier differences — SQL invocation
Heavyweight registers `gbx_rst_custom_rastertogridcount` as a scalar (ARRAY-returning) function — call it directly and `explode(...)` to flatten to rows. The lightweight (pyrx) tier registers it as a streaming **table function**, so lightweight SQL must use `LATERAL`. Both forms appear in the SQL tab below.
:::

**Signature:** `rst_custom_rastertogridcount(tile: Column, grid: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column` — Pixel count (Double) per custom-grid cell.

**SQL:**

<FunctionExamples
  name="rst_custom_rastertogridcount"
  sql={rasterxSqlCode}                  sqlSource="docs/tests/python/api/rasterx_functions_sql.py"
  pythonLight={rasterxGridaggLightCode} pythonLightSource="docs/tests/python/api/rasterx_gridagg_python_light.py"
  pythonHeavy={rasterxCode}             pythonHeavySource="docs/tests/python/api/rasterx_functions.py"
  scala={scalaApiExamplesCode}          scalaSource="docs/tests/scala/api/ScalaApiExamples.scala"
...
Maximum pixel value per custom-grid cell.

:::note Tier differences — SQL invocation
Heavyweight registers `gbx_rst_custom_rastertogridmax` as a scalar (ARRAY-returning) function — call it directly and `explode(...)` to flatten to rows. The lightweight (pyrx) tier registers it as a streaming **table function**, so lightweight SQL must use `LATERAL`. Both forms appear in the SQL tab below.
:::

**Signature:** `rst_custom_rastertogridmax(tile: Column, grid: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column` — Max pixel value per custom-grid cell.

**SQL:**

<FunctionExamples
  name="rst_custom_rastertogridmax"
  sql={rasterxSqlCode}                  sqlSource="docs/tests/python/api/rasterx_functions_sql.py"
  pythonLight={rasterxGridaggLightCode} pythonLightSource="docs/tests/python/api/rasterx_gridagg_python_light.py"
  pythonHeavy={rasterxCode}             pythonHeavySource="docs/tests/python/api/rasterx_functions.py"
  scala={scalaApiExamplesCode}          scalaSource="docs/tests/scala/api/ScalaApiExamples.scala"
...
Median pixel value per custom-grid cell.

:::note Tier differences — SQL invocation
Heavyweight registers `gbx_rst_custom_rastertogridmedian` as a scalar (ARRAY-returning) function — call it directly and `explode(...)` to flatten to rows. The lightweight (pyrx) tier registers it as a streaming **table function**, so lightweight SQL must use `LATERAL`. Both forms appear in the SQL tab below.
:::

**Signature:** `rst_custom_rastertogridmedian(tile: Column, grid: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column` — Median pixel value per custom-grid cell.

**SQL:**

<FunctionExamples
  name="rst_custom_rastertogridmedian"
  sql={rasterxSqlCode}                  sqlSource="docs/tests/python/api/rasterx_functions_sql.py"
  pythonLight={rasterxGridaggLightCode} pythonLightSource="docs/tests/python/api/rasterx_gridagg_python_light.py"
  pythonHeavy={rasterxCode}             pythonHeavySource="docs/tests/python/api/rasterx_functions.py"
  scala={scalaApiExamplesCode}          scalaSource="docs/tests/scala/api/ScalaApiExamples.scala"
...
Minimum pixel value per custom-grid cell.

:::note Tier differences — SQL invocation
Heavyweight registers `gbx_rst_custom_rastertogridmin` as a scalar (ARRAY-returning) function — call it directly and `explode(...)` to flatten to rows. The lightweight (pyrx) tier registers it as a streaming **table function**, so lightweight SQL must use `LATERAL`. Both forms appear in the SQL tab below.
:::

**Signature:** `rst_custom_rastertogridmin(tile: Column, grid: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column` — Min pixel value per custom-grid cell.

**SQL:**

<FunctionExamples
  name="rst_custom_rastertogridmin"
  sql={rasterxSqlCode}                  sqlSource="docs/tests/python/api/rasterx_functions_sql.py"
  pythonLight={rasterxGridaggLightCode} pythonLightSource="docs/tests/python/api/rasterx_gridagg_python_light.py"
  pythonHeavy={rasterxCode}             pythonHeavySource="docs/tests/python/api/rasterx_functions.py"
  scala={scalaApiExamplesCode}          scalaSource="docs/tests/scala/api/ScalaApiExamples.scala"
...
Population standard deviation (`sqrt` of the population variance) of pixel values per custom-grid cell — a single-pixel cell yields `0.0`.

:::note Tier differences — SQL invocation
Heavyweight registers `gbx_rst_custom_rastertogridstddev` as a scalar (ARRAY-returning) function — call it directly and `explode(...)` to flatten to rows. The lightweight (pyrx) tier registers it as a streaming **table function**, so lightweight SQL must use `LATERAL`. Both forms appear in the SQL tab below.
:::

**Signature:** `rst_custom_rastertogridstddev(tile: Column, grid: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column` — Population standard deviation of pixel values per custom-grid cell.

**SQL:**

<FunctionExamples
  name="rst_custom_rastertogridstddev"
  sql={rasterxSqlCode}                  sqlSource="docs/tests/python/api/rasterx_functions_sql.py"
  pythonLight={rasterxGridaggLightCode} pythonLightSource="docs/tests/python/api/rasterx_gridagg_python_light.py"
  pythonHeavy={rasterxCode}             pythonHeavySource="docs/tests/python/api/rasterx_functions.py"
  scala={scalaApiExamplesCode}          scalaSource="docs/tests/scala/api/ScalaApiExamples.scala"
...
Sum of pixel values per custom-grid cell.

:::note Tier differences — SQL invocation
Heavyweight registers `gbx_rst_custom_rastertogridsum` as a scalar (ARRAY-returning) function — call it directly and `explode(...)` to flatten to rows. The lightweight (pyrx) tier registers it as a streaming **table function**, so lightweight SQL must use `LATERAL`. Both forms appear in the SQL tab below.
:::

**Signature:** `rst_custom_rastertogridsum(tile: Column, grid: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column` — Sum of pixel values per custom-grid cell.

**SQL:**

<FunctionExamples
  name="rst_custom_rastertogridsum"
  sql={rasterxSqlCode}                  sqlSource="docs/tests/python/api/rasterx_functions_sql.py"
  pythonLight={rasterxGridaggLightCode} pythonLightSource="docs/tests/python/api/rasterx_gridagg_python_light.py"
  pythonHeavy={rasterxCode}             pythonHeavySource="docs/tests/python/api/rasterx_functions.py"
  scala={scalaApiExamplesCode}          scalaSource="docs/tests/scala/api/ScalaApiExamples.scala"
...
Population variance (`÷ n`, two-pass) of pixel values per custom-grid cell — a single-pixel cell yields `0.0`.

:::note Tier differences — SQL invocation
Heavyweight registers `gbx_rst_custom_rastertogridvariance` as a scalar (ARRAY-returning) function — call it directly and `explode(...)` to flatten to rows. The lightweight (pyrx) tier registers it as a streaming **table function**, so lightweight SQL must use `LATERAL`. Both forms appear in the SQL tab below.
:::

**Signature:** `rst_custom_rastertogridvariance(tile: Column, grid: Column, resolution: Column, [coverage: Column], [assignment: Column]): Column` — Population variance of pixel values per custom-grid cell.

**SQL:**

<FunctionExamples
  name="rst_custom_rastertogridvariance"
  sql={rasterxSqlCode}                  sqlSource="docs/tests/python/api/rasterx_functions_sql.py"
  pythonLight={rasterxGridaggLightCode} pythonLightSource="docs/tests/python/api/rasterx_gridagg_python_light.py"
  pythonHeavy={rasterxCode}             pythonHeavySource="docs/tests/python/api/rasterx_functions.py"
  scala={scalaApiExamplesCode}          scalaSource="docs/tests/scala/api/ScalaApiExamples.scala"
...
:::note Lightweight tier (pyrx)
Powered by **rasterio** (`rasterio.warp`). Warps `tile` onto the reference tile's grid — matching its CRS, extent, and pixel dimensions — using nearest-neighbour resampling. Satisfies the alignment precondition required by the `rst_combine*` family.
:::

Warp a tile to match the grid of a reference tile exactly — same CRS, extent, width, height, and geotransform — using nearest-neighbour resampling (`-r near`). This is the explicit fix for the alignment precondition of the cross-raster combine family (`rst_combine*`): align each tile to a common reference, then combine.

**Signature:** `rst_align_to(tile: Column, reference_tile: Column): Column`

**Parameters:** `tile` — input raster tile to warp; `reference_tile` — reference tile whose grid (CRS, extent, pixel size) defines the output grid.
...
:::note Lightweight tier (pyrx)
Powered by **rasterio** + **NumPy**. Computes per-band percentiles over valid pixels, clips to that range, and linearly rescales to [0,255]. Each band is stretched independently.
:::
...
**Signature:** `rst_percentile_stretch(tile: Column, lo_pct: Column, hi_pct: Column): Column` — Per-band percentile contrast stretch to uint8. For each band, `lo_pct` and `hi_pct` (percentiles in 0–100) define the stretch range: pixels at the lower percentile map to 0, at the upper percentile map to 255, and intermediate pixels are linearly rescaled. NoData pixels remain 0 in the output. Typical values: `lo_pct=2.0, hi_pct=98.0` (trim outliers and stretch to full uint8 range). Output is a uint8 GTiff with the same bands, extent, and CRS as the input.

<FunctionExamples
  name="rst_percentile_stretch"
  sql={rasterxSqlCode}                  sqlSource="docs/tests/python/api/rasterx_functions_sql.py"
  pythonLight={rasterxTileopsLightCode}  pythonLightSource="docs/tests/python/api/rasterx_tileops_python_light.py"
  pythonHeavy={rasterxCode}             pythonHeavySource="docs/tests/python/api/rasterx_functions.py"
  scala={scalaApiExamplesCode}          scalaSource="docs/tests/scala/api/ScalaApiExamples.scala"
...
Per-pixel mean across an `ARRAY` of aligned tiles. **On invocation** the tiles are read as a pixel-aligned stack — they must share grid, extent, and CRS, so align them with `rst_align_to` first — and each output pixel is the mean of that location's valid (non-NoData) values across the stack (a pixel that is NoData in *every* input stays NoData). Use for temporal/ensemble composites such as cloud-reduced mean reflectance.
...
:::note Lightweight tier (pyrx)
Powered by **rasterio** + **NumPy**. Takes an `ARRAY<tile>` and returns a tile where each pixel is the count of valid (non-NoData) inputs across the stack; input tiles must share the same grid (shape/extent/CRS). All-NoData pixels yield NoData in the output (never 0 for an all-NoData pixel). Align tiles with `gbx_rst_align_to` before combining.
:::

Per-pixel count of valid (non-NoData) inputs across an `ARRAY` of aligned tiles. **On invocation** the tiles are read as a pixel-aligned stack — they must share grid, extent, and CRS, so align them with `rst_align_to` first — and each output pixel is the number of inputs whose value at that location is valid; a location that is NoData in *every* input yields NoData (never 0). Use alongside the other `rst_combine*` reducers to build multi-statistic temporal or ensemble composites.

**Signature:** `rst_combinecount(tiles: Column): Column` — Count of valid pixels per location.

<FunctionExamples
  name="rst_combinecount"
  sql={rasterxSqlCode}                  sqlSource="docs/tests/python/api/rasterx_functions_sql.py"
  pythonLight={rasterxBandmathLightCode} pythonLightSource="docs/tests/python/api/rasterx_bandmath_python_light.py"
  pythonHeavy={rasterxCode}             pythonHeavySource="docs/tests/python/api/rasterx_functions.py"
  scala={scalaApiExamplesCode}          scalaSource="docs/tests/scala/api/ScalaApiExamples.scala"
...
:::note Lightweight tier (pyrx)
Powered by **rasterio** + **NumPy**. Takes an `ARRAY<tile>` and returns the NoData-aware per-pixel maximum; input tiles must share the same grid (shape/extent/CRS). All-NoData pixels yield NoData in the output. Align tiles with `gbx_rst_align_to` before combining.
:::

Per-pixel maximum across an `ARRAY` of aligned tiles. **On invocation** the tiles are read as a pixel-aligned stack — they must share grid, extent, and CRS, so align them with `rst_align_to` first — and each output pixel is the largest of that location's valid (non-NoData) values across the stack (a pixel that is NoData in *every* input stays NoData). Use to build cloud-free maximum-NDVI composites or storm-peak accumulations.

**Signature:** `rst_combinemax(tiles: Column): Column` — Maximum pixel value per location.
...
:::note Lightweight tier (pyrx)
Powered by **rasterio** + **NumPy**. Takes an `ARRAY<tile>` and returns the NoData-aware per-pixel median; input tiles must share the same grid (shape/extent/CRS). Even-count median uses the mean of the two middle values (`numpy np.ma.median` convention). All-NoData pixels yield NoData in the output. Align tiles with `gbx_rst_align_to` before combining.
:::

Per-pixel median across an `ARRAY` of aligned tiles. **On invocation** the tiles are read as a pixel-aligned stack — they must share grid, extent, and CRS, so align them with `rst_align_to` first — and each output pixel is the median of that location's valid (non-NoData) values across the stack (even counts average the two middle values; a pixel that is NoData in *every* input stays NoData). Robust to outliers — useful for seasonal median composites from time-series imagery.

**Signature:** `rst_combinemedian(tiles: Column): Column` — Median pixel value per location.

<FunctionExamples
  name="rst_combinemedian"
  sql={rasterxSqlCode}                  sqlSource="docs/tests/python/api/rasterx_functions_sql.py"
  pythonLight={rasterxBandmathLightCode} pythonLightSource="docs/tests/python/api/rasterx_bandmath_python_light.py"
  pythonHeavy={rasterxCode}             pythonHeavySource="docs/tests/python/api/rasterx_functions.py"
  scala={scalaApiExamplesCode}          scalaSource="docs/tests/scala/api/ScalaApiExamples.scala"
...
:::note Lightweight tier (pyrx)
Powered by **rasterio** + **NumPy**. Takes an `ARRAY<tile>` and returns the NoData-aware per-pixel minimum; input tiles must share the same grid (shape/extent/CRS). All-NoData pixels yield NoData in the output. Align tiles with `gbx_rst_align_to` before combining.
:::

Per-pixel minimum across an `ARRAY` of aligned tiles. **On invocation** the tiles are read as a pixel-aligned stack — they must share grid, extent, and CRS, so align them with `rst_align_to` first — and each output pixel is the smallest of that location's valid (non-NoData) values across the stack (a pixel that is NoData in *every* input stays NoData). Use to extract the lowest-cloud-probability pixel from a time stack or to track cumulative minimum temperatures.

**Signature:** `rst_combinemin(tiles: Column): Column` — Minimum pixel value per location.
...
:::note Lightweight tier (pyrx)
Powered by **rasterio** + **NumPy**. Takes an `ARRAY<tile>` and returns the NoData-aware per-pixel population standard deviation (ddof=0) across the stack; input tiles must share the same grid (shape/extent/CRS). All-NoData pixels yield NoData in the output. Align tiles with `gbx_rst_align_to` before combining.
:::

Per-pixel population standard deviation across an `ARRAY` of aligned tiles. **On invocation** the tiles are read as a pixel-aligned stack — they must share grid, extent, and CRS, so align them with `rst_align_to` first — and each output pixel is the population standard deviation (ddof=0) of that location's valid (non-NoData) values across the stack (a pixel that is NoData in *every* input stays NoData). Quantifies variability in a time-series or multi-scene ensemble — high values indicate heterogeneous pixels.

**Signature:** `rst_combinestddev(tiles: Column): Column` — Population standard deviation of pixel values per location.

<FunctionExamples
  name="rst_combinestddev"
  sql={rasterxSqlCode}                  sqlSource="docs/tests/python/api/rasterx_functions_sql.py"
  pythonLight={rasterxBandmathLightCode} pythonLightSource="docs/tests/python/api/rasterx_bandmath_python_light.py"
  pythonHeavy={rasterxCode}             pythonHeavySource="docs/tests/python/api/rasterx_functions.py"
  scala={scalaApiExamplesCode}          scalaSource="docs/tests/scala/api/ScalaApiExamples.scala"
...
:::note Lightweight tier (pyrx)
Powered by **rasterio** + **NumPy**. Takes an `ARRAY<tile>` and returns the NoData-aware per-pixel sum across the stack; input tiles must share the same grid (shape/extent/CRS). All-NoData pixels yield NoData in the output. Align tiles with `gbx_rst_align_to` before combining.
:::

Per-pixel sum across an `ARRAY` of aligned tiles. **On invocation** the tiles are read as a pixel-aligned stack — they must share grid, extent, and CRS, so align them with `rst_align_to` first — and each output pixel is the sum of that location's valid (non-NoData) values across the stack (a pixel that is NoData in *every* input stays NoData). Use to accumulate total precipitation from a daily time-series or to sum spectral flux across dates.

**Signature:** `rst_combinesum(tiles: Column): Column` — Sum of pixel values per location.
...
:::tip Esri Resample / changing cell size
...
- **[`rst_resample_to_res`](#rst_resample_to_res)** — by target **ground resolution** in CRS units (e.g. metres per pixel).
- **[`rst_resample_to_size`](#rst_resample_to_size)** — by target **pixel dimensions** (width × height).

Unlike [`rst_sample`](#rst_sample) — which reads a value at a point and is nearest-pixel only — resampling rewrites the whole grid. So to *sample* with **bilinear or cubic interpolation**, resample first with the desired `algorithm`, then call `rst_sample`.

See [Raster Sampling](./raster-sampling) for the full workflow guide.
:::
...
Part of the `rst_resample*` family — see the [Esri Resample note](#rst_resample) for how it relates to the ArcGIS Resample tool and `rst_sample`.
...
:::note Lightweight-only `xscale` / `yscale` parameters
