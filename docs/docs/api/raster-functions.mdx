...
Tile-returning functions (`rst_clip`, `rst_resample`, `rst_transform`, the aggregators, the constructors, …) return a **[v2 Tile](./tile-structure)**. Their output is shown as a representative struct — `{0, <raster bytes>, <virtual path>, {driver -> GTiff, ...}}` — rather than a literal byte dump. The lightweight tier defaults to [**virtual tiles**](./virtual-tiles): the tile carries a `path` and reads its raster bytes lazily, so a populated `path` in the output is expected.
