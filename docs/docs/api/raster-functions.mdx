...
| Placeholder | Sample file | Temp view | Backs |
|---|---|---|---|
| `GTIFF_SAMPLE_DIR` | single-band GeoTIFF | `rasters` | **Default** — most accessor, tile-ops, transform, and generator examples |
| `GTIFF_MULTI_DIR` | multi-band GeoTIFF (red/NIR/green) | `multiband_rasters` | band-math and spectral-index examples, `rst_numbands`, `rst_bandmetadata` |
| `DTM_DIR` | digital elevation model | `dem_rasters` | terrain examples (`rst_slope`, `rst_aspect`, …) |
| `NETCDF_DIR` | NetCDF with subdatasets | `netcdf_rasters` | `rst_subdatasets`, `rst_getsubdataset` |
