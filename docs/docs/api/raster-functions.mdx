...
- **Grid Functions (quadbin)**: Aggregate raster values to CARTO quadbin v0 cells (rastertogrid avg/count/max/min/median; tessellate; rasterize_agg)
- **Grid Functions (BNG)**: Aggregate raster values to British National Grid cells (rastertogrid avg/count/max/min/median; tessellate; rasterize_agg)
...
<Tier heavy/> <Impl groupedAgg/>

:::note Heavyweight tier only
Quadbin rasterize aggregation is currently **heavy-tier only**. Lightweight support is planned for a future release.
:::
...
Heavyweight `gbx_rst_quadbin_rasterize_agg` returns a tile **`STRUCT<cellid, raster, metadata>`**.
...
:::note Heavyweight tier only
BNG rasterize aggregation is currently **heavy-tier only**. Lightweight support is planned for a future release (pending `pygx` BNG raster integration).
:::
...
Heavyweight `gbx_rst_bng_rasterize_agg` returns a tile **`STRUCT<cellid, raster, metadata>`**.
...
<Tier heavy/> <Impl udtf/>

:::note Heavyweight tier only
Tessellate to CARTO quadbin v0 cells is currently **heavy-tier only**. Lightweight support is planned for a future release.
:::
...
:::note Heavyweight tier only
Tessellate to BNG cells is currently **heavy-tier only**. Lightweight support is planned for a future release (pending `pygx` BNG raster integration).
:::
...
Aggregate raster values to British National Grid (BNG) cells (5 total). BNG functions automatically reproject the input raster to **EPSG:27700** before sampling — no `rst_transform` needed upstream. Resolution accepts integer indices **±1..±6** (1 = 100 km, 2 = 10 km, 3 = 1 km, 4 = 100 m, 5 = 10 m, 6 = 1 m; negative indices address quadrant sub-cells) or string keys (e.g. `"1km"`, `"100m"`). Cell IDs are **STRING**.

:::note Heavy-tier only
The BNG raster-grid functions are currently heavy-tier only. Lightweight support is planned for a future release (pending `pygx` BNG raster integration).
:::
