...
:::info Opt-in extra
`StacClient` requires `geobrix[light_env6,stac]`. The `[stac]` extra pulls in `pystac-client`, `planetary-computer`, `tenacity`, and `requests`. Serverless environment version 6 (Python 3.12) is required.
:::
...
```bash
pip install "geobrix[light_env6,stac]"
```

From a Databricks notebook (Serverless or classic):

```python
%pip install --quiet "geobrix[light_env6,stac] @ file:///Volumes/<catalog>/<schema>/<volume>/geobrix-0.5.0-py3-none-any.whl"
```
