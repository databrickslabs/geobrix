...
- **Coverage validity** — `gbx_st_coverageisvalid`, `gbx_st_coverageinvalidedges` (SQL grouped-aggregates), and `coverage_simplify` (Python-API helper) for validating and simplifying polygon coverages, **lightweight tier only** (`pyvx`)
...
|---|---|---|
| Install | Volume-staged `[light_env6]` wheel ([install](../installation)) | Init script + JAR |
...
Distributed shapely helpers — grouped-aggregate SQL UDFs and a Python DataFrame helper — for validating and simplifying polygon **coverages** (sets of polygons that share edges and together partition a region). **Lightweight tier only** (`pyvx`); no heavyweight counterpart exists. See [Coverage Validity](./coverage-validity) for the `gap_width` semantics, scale-ceiling notes, and the comparison with per-geometry validity.
...
See [Coverage Validity](./coverage-validity).

<FunctionExamples
  name="st_coverageisvalid"
  sql={vectorxSqlCode}               sqlSource="docs/tests/python/api/vectorx_functions_sql.py"
  pythonLight={vectorxLightCode}     pythonLightSource="docs/tests/python/api/vectorx_functions_python_light.py"
  testFile="docs/tests/python/api/test_vectorx_functions_sql.py"
/>
...
See [Coverage Validity](./coverage-validity#coverage_simplify-python-api).
...
- [Coverage Validity](./coverage-validity) — validate and simplify polygon coverages with `gbx_st_coverageisvalid`, `gbx_st_coverageinvalidedges`, and `coverage_simplify`
