...
**Signature:** `st_asmvt(geom: Column, attrs: Column, layerName: Column): Column`

**Parameters:**
- `geom` (`BINARY`) — Feature geometry (WKB, EWKB, WKT, or EWKT) in **tile-local coordinates**. Compose any `ST_Intersection` against the tile envelope and coordinate translation upstream.
...
**Signature (SQL):** `gbx_st_asmvt_pyramid(geom, attrs, min_z, max_z, layer_name, extent)`

**Parameters:**

- `geom` (`BINARY`) — Feature geometry (WKB, EWKB, WKT, or EWKT) in **EPSG:4326 lon/lat**. The UDTF
...
**Signature:** `st_asmvt_pyramid(geom: Column, attrs: Column, minZoom: Column, maxZoom: Column, layerName: Column): Column`

**Parameters:**
- `geom` (`BINARY`) — Feature geometry (WKB, EWKB, WKT, or EWKT) in **EPSG:4326 lon/lat**. The function performs the per-tile clip and tile-local coordinate transform; no upstream `ST_Intersection` required.
- `attrs` (`STRUCT<...>`) — Per-feature attributes. Integer, long, double, and boolean fields are encoded as native MVT value types; other types are encoded as strings.
