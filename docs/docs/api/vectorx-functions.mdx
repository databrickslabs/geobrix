...
**Z coordinates.** A geometry whose vertices *all* carry a finite Z keeps its Z. A genuinely 2D geometry stays 2D — no Z ordinate is invented. For a geometry where only *some* vertices carry a Z, the current behavior is that `gbx_st_transformcrs` reprojects it as **2D**: reprojecting a missing Z would propagate it into X and Y and destroy the horizontal position, so the Z is dropped and every X/Y stays correct. A missing Z is never filled in with a substitute value such as `0`, which would be indistinguishable from a surveyed elevation downstream.
...
**Returns:** `BINARY` — EWKB with the new SRID embedded. Coordinates are byte-preserved.
