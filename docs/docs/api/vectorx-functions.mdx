...
- **M (measure) values are dropped.** Both tiers carry X, Y and Z only, so a `ZM` geometry comes back as `Z` with the measure gone. This is worth noting because the product's own geometry type does persist M.
:::
