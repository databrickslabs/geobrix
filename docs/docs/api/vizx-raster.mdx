...
| `limit` | `int` or `None` | mode default | Maximum rows to pull to the driver. Defaults to 25 (facet), 1 (first), or 64 (mosaic). If the DataFrame has more rows than the limit, a warning is issued and only the first `limit` tiles are rendered — the entire DataFrame is never collected. |
| `fig_w` | `float` | `10` | Figure width in inches. |
...
- `"mosaic"` — merges tiles into a single georeferenced image via `rasterio.merge`. All tiles must share the same CRS; a `ValueError` is raised if the collected rows contain more than one distinct CRS.
...
plot_tiles(df.filter("cellid is not null"), mode="facet", limit=4)

# Inspect a single tile quickly:
plot_tiles(df.filter("path = '/Volumes/main/samples/sentinel/B04.tif'"), mode="first")
