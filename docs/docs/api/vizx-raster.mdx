...
```python
plot_raster(raster, *, fig_w=10, fig_h=10, max_pixels=2000, composite="auto",
            bands=None, stretch="perband", fill=None, cmap=None)
```

Render a raster from in-memory bytes **or a tile struct** (materialized or virtual):

- **bytes / bytearray** — the classic path; pass a tile's `raster` field directly.
- **v2 tile struct** (dict or Spark Row) — when `raster` is set, its bytes are used.  When `raster` is `None` and `path` is set, the file is opened on-the-fly and the tile's `window` (`col_off`, `row_off`, `width`, `height`) is used to read only that region.  This is the memory-efficient **virtual tile** path — no bytes are loaded until rendering.  `window=None` reads the whole file.

```python
from databricks.labs.gbx.vizx import plot_raster

# Classic: pass raster bytes
row = df.select("tile").first()
plot_raster(row["tile"]["raster"])

# Virtual tile: pass the whole tile struct — bytes stay on disk
plot_raster(row["tile"])

# Coverage-depth view of a multi-band presence-mask stack:
plot_raster(row["tile"]["raster"], composite="depth")

# DEM with terrain colormap:
plot_raster(row["tile"], cmap="terrain")
```

The `cmap` parameter sets the colormap for single-band renders (default `None` → viridis). It has no effect on RGB composites.
...
# DEM with terrain colormap:
plot_file("/Volumes/main/geobrix_samples/geobrix-examples/nyc/dem.tif", cmap="terrain")
...
The `cmap` parameter sets the colormap for single-band renders (default `None` → viridis).
