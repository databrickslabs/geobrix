...
| `max_rows` | `int` or `None` | `10_000` | Row cap for driver-side collects; `None` collects all rows. Not applied on the product-dissolve path (see `dissolve_engine`). |
| `dissolve_by` | `str` or `None` | `None` | When set, dissolve cell polygons by this column, returning one footprint polygon per distinct value. Must be in `extra_cols`. |
| `dissolve_engine` | `str` | `"auto"` | Controls how the dissolve is performed when `dissolve_by` is set. See the engine table below. |
| `sample_seed` | `int` or `None` | `None` | `None` (default) takes the first `max_rows` rows (first-N, not a representative sample). An int draws a reproducible random sample via `DataFrame.sample` at the cost of one extra `count()` job. |
