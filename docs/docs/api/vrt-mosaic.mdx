...
| `gridSystem` | `"none"` | Tile grid. `"none"` (native pixel tiling, default) or `"quadbin"` (cell-aligned quadbin grid, reprojected to EPSG:3857). BNG and H3 are planned for a future release. |
...
BNG and H3 grid-aligned mosaics are planned for a future release — tiles aligned
to those grid cells and tagged with a cell identifier for spatial joins and data
unification. In-notebook windowed rendering of a VRT mosaic is also on the
roadmap. Pyramid options (`gridMinResolution`, `gridMaxResolution`,
`gridStepResolution`) are accepted by the parser but raise a clear error until
their grid-system support lands.
