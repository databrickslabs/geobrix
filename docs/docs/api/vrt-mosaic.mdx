...
BNG grid-aligned mosaics are planned — tiles aligned to BNG cells and tagged
with a cell identifier for spatial joins and data unification. In-notebook
windowed rendering of a VRT mosaic is also on the roadmap. Pyramid options
(`gridMinResolution`, `gridMaxResolution`, `gridStepResolution`) are accepted
by the parser but raise a clear error until their grid-system support lands.
