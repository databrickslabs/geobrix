...
Pass `mosaic=true` and `gridSystem=none` to `cog_gbx` to enter mosaic mode.
...
|---|---|---|
| `mosaic` | — | Set `"true"` to activate mosaic mode. Either `mosaic` or `gridSystem` must be present. |
