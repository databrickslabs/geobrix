...
These functions use only FUSE (`os.walk` / `stat`) and require no `SparkSession`. They are the
enumeration and path-normalization primitives every DataSource reader consumes internally.
They are **Connect-safe**: no `sparkContext`, `.rdd`, `_jvm`, or `conf.set`.
...
| `enumerate_files` | `enumerate_files(path, *, recursive=True, include_hidden=False, extensions=None, path_glob_filter=None, spark=None)` | DataFrame `[path, size, file]` or `list[dict]` | Session-ful when FILE is available (returns a Spark DataFrame); falls back to a FUSE list of dicts otherwise. |
