...
- Optional: `GBX_BUNDLE_WHEEL_VOLUME_PATH` — path to a GeoBrix wheel on a Volume (e.g. `.../geobrix-0.2.0-py3-none-any.whl`). When set, the bundle runner notebook gets two pip cells plus `dbutils.library.restartPython()` so the cluster loads the latest code.
- Optional: `GBX_BUNDLE_SKIP_WHEEL_UPLOAD=1` — use an existing wheel (no build/upload); notebook still gets the pip and restart cells.
...
| `gbx:data:push-wheel` | Put built wheel on a Volume for cluster install | Builds Python wheel and uploads to `GBX_BUNDLE_WHEEL_VOLUME_PATH` |
