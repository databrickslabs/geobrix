...
| **An interactive chart that filters the map (the "halo")** | *"Chart … and let me filter the map"* — e.g. *"Chart plume concentration by operator and let me filter the map"* | **one row per feature**, carrying **both** a `*_geojson` geometry **and** chartable attributes (a number like `max_conc_ppmm`, plus categoricals like `lead_operator`) | a chart whose selection highlights the matching features on the map, and vice-versa |

The distinction is the row grain. A **map-layer** answer just needs geometry. The **halo**
needs geometry *and* per-feature attributes on the same rows — because the chart and the
map layer are the same dataset, and selecting chart elements cross-filters the layer by
row. A pre-aggregated rollup (one row per operator, per county, per H3 cell) still charts
fine, but with no per-feature geometry it can't light up the map — so when you want the
cross-filter, ask for the individual features, not the summary.
