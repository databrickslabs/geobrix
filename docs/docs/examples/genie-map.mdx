...
| **An interactive chart that filters the map** | *"Chart … and let me filter the map"* — e.g. *"Chart plume concentration by operator and let me filter the map"* | **one row per feature**, carrying **both** a `*_geojson` geometry **and** chartable attributes (a number like `max_conc_ppmm`, plus categoricals like `lead_operator`) | a chart whose selection highlights the matching features on the map, and vice-versa |
