...
- **Virtual-tile ingestion — bytes-free.** The `gtiff_gbx` reader loads each Sentinel-2 scene as [virtual tiles](../api/virtual-tiles): with `.option("tileSize", "2048,2048")` a 5490×5490 scene fans into ~9 windows, and each tile row carries a source `path` + pixel `window` **instead of raster bytes** — so `band_b0X_tile` is a table of lightweight references (`tile.raster` is `null`; `tile.path`/`tile.window` set), no pixels in Delta. `rst_initnodata` records a pending nodata instruction and `rst_srid`/`rst_boundingbox` answer from the header, all without materializing.
- **Spatial-indexed raster tables (the materialization cut-over)** — `rst_h3_tessellate` shreds each scene into H3 resolution-7 cells, producing `band_b0X_h3` Delta tables that join cleanly across bands and dates. This is where the virtual tiles **materialize**: each window's pixels are read once, at tessellation.
