...
- **[`h3_try_coverash3`](https://docs.databricks.com/aws/en/sql/language-manual/functions/h3_try_coverash3)** — Databricks built-in H3 function, accessed via the product Python bindings (`from pyspark.databricks.sql import functions as DBF`; available DBR 16.3+ / Serverless). Returns all H3 cells that overlap the polygon — solid, gap-free bands. Accepts WKB BINARY geometry directly — no `ST_GeomFromWKB` needed. The `h3_try_*` prefix returns `NULL` instead of raising on invalid input. (`h3_polyfillash3` is the centroid-in alternative for cases where a sparser subset is preferred.)
