...
- **GeoBrix** (version 0.4.0). The `%pip install` cell installs the `geobrix[light,vizx]` wheel, which pulls in `rasterio` (used for the driver-side DEM read and isoband extraction), `h3` (polyfill), `matplotlib`, `geopandas`, and `mapclassify` (visualization). No JAR or GDAL init script is required.
...
- **Wheel path**: update the `%pip install` cell to point at your staged `geobrix-0.4.1-py3-none-any.whl` if its Volume path differs from the default.
