...
:::info Runs on the lightweight tier (`geobrix[light_env5,stac,vizx,overture]`)
...
- **Unity Catalog Volumes**: `DemDownloader.lidar_dtm()` stages USGS 3DEP LiDAR-DTM (≈2 m) GeoTIFFs to `/Volumes/geospatial_docs/geobrix/sample-data/geobrix-examples/sf/elevation-3dep-lidar/dem_2m`; `OvertureClient` stages the Overture water layer to `.../sf/overture-water`. Update `DEM_DIR` and `LAND_DIR` if your Volume layout differs. The Volume root must already exist; staging creates sub-directories automatically.
...
3. **Download DEM and land mask** — `DemDownloader.lidar_dtm().download(…)` fetches USGS 3DEP LiDAR-DTM (≈2 m) tiles via Planetary Computer STAC and stages them (idempotent); `OvertureClient` fetches Overture Maps `base/water` features and builds a land multipolygon by subtracting water from the AOI box.
4. **Steps 1–3 (Spark)** — the `raster_gbx` reader with `tileSize=512` splits the DEM into virtual tiles; `rst_clip` masks each tile to the land polygon; `rst_isoband` extracts twelve 25 m elevation bands; `h3_try_coverash3` indexes each band at H3 res 10 (Databricks product function, accepts WKB directly); `rst_h3_gridspec` computes the shared canvas.
...
```text
DemDownloader.lidar_dtm()  →  USGS 3DEP LiDAR-DTM (≈2 m)  (Planetary Computer STAC → UC Volume)
OvertureClient             →  base/water features → land multipolygon (EPSG:4326)
        ...
        │
        ▼  rst_transform → EPSG:4326  (lidar-dtm is UTM zone 10N; align for the 4326 clip + lon/lat H3)
        │
...
- **`DemDownloader.lidar_dtm()`** — selects the USGS **3DEP LiDAR-DTM (≈2 m)** product; `.download(bbox, path, resolution="finest")` queries the Planetary Computer STAC for tiles intersecting the bounding box and writes GeoTIFFs to a UC Volume (idempotent). See [DEM downloader](../sample-data/dem-downloader).
...
- **`raster_gbx` reader with `tileSize`** — the `spark.read.format("raster_gbx").option("tileSize","512").load(path)` call splits each GeoTIFF into 512×512-px **virtual tiles**: bytes-free structs (path + window) read lazily. One Spark row per tile; the ≈2 m DEM is processed in parallel across executors.
- **`rst_transform`** — reprojects each tile to a target EPSG. The 3DEP LiDAR-DTM tiles are in a projected CRS (UTM zone 10N over SF), so `rx.rst_transform("tile", 4326)` brings them to lon/lat — required for the EPSG:4326 land cutline and product H3 (which indexes lon/lat). Identity (already 4326, e.g. the seamless product) is a no-op passthrough. See [RasterX functions](../api/raster-functions).
- **`rst_clip`** — masks each tile to a vector cutline (WKB polygon), setting all pixels outside the cutline to NoData. `cutline_all_touched=True` keeps pixels the boundary touches; `clip_crs` defaults to the raster CRS (here EPSG:4326 after the reproject). See [RasterX functions](../api/raster-functions).
...
- **Volume write is serverless-safe**: `DemDownloader.lidar_dtm()` stages AOI-windowed 3DEP GeoTIFFs and `OvertureClient` stages Overture Parquet files directly to Unity Catalog Volume paths using sequential I/O — no `seek` and no unbounded in-memory buffering. Both operations are idempotent and safe on Serverless compute.
