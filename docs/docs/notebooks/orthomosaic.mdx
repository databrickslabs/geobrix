...
output is written as a GeoTIFF, converted to a COG with `rst_cog_convert`, and packaged as
...
### `01_sfm_orthomosaic` — telemetry → SfM → orthomosaic
...
### `02_color_correction` — per-channel percentile stretch

Reads `orthomosaic.tif` and applies a per-channel percentile stretch (configurable
low/high clip percentiles) to produce `orthomosaic_corrected.tif` with balanced contrast.

### `03_cog` — cloud-optimised layout

Converts `orthomosaic_corrected.tif` to a Cloud-Optimised GeoTIFF with internal tiling
and overview levels via `rst_cog_convert`, producing `orthomosaic_cog.tif`. The COG
layout enables efficient range-request reads downstream and is the input to the PMTiles
step.

### `04_pmtiles` — PMTiles for browser preview

Reads `orthomosaic_cog.tif` and encodes it as a self-contained PMTiles archive
(`orthomosaic.pmtiles`) using the GeoBrix PMTiles writer. The resulting archive can be
previewed interactively in a browser tab (PMTiles + MapLibre GL or Leaflet) with no tile
server.
...
reconstruction statistics (registered images, 3-D points) while `01_sfm_orthomosaic` runs.
...
- **`rst_cog_convert`** — `rst_cog_convert(tile)` converts a materialized raster tile to
  COG layout (internal tiling + overviews) in a single Spark expression. See
  [RasterX functions](../api/raster-functions).
...
Drone JPEGs (local or Volume)
        ...
        │
        ▼  rst_cog_convert — orthomosaic_cog.tif (COG layout)
        │
