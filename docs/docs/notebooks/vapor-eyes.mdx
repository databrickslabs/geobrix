...
on Databricks. It comes in two flavors: a **production Lakeflow pipeline** that runs on a
schedule and stays current, and an **interactive notebook series** that walks the same
detection cascade step by step.
