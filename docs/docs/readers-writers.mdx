...
_Parenthetical `(x.y.z)` is the GeoBrix release each reader/writer first shipped (its "since" version)._

| Format | Lightweight | Heavyweight |
|---|---|---|
| [Raster (generic)](./readers/raster) | `raster_gbx` (0.4.0) | `gdal` (0.2.0) |
| [GeoTIFF](./readers/geotiff) | `gtiff_gbx` (0.4.0) | `gtiff_gdal` (0.2.0) |
| [COG](./readers/cog) | `cog_gbx` (0.5.0) | — (light-only) |
| [NetCDF](./readers/netcdf) | `netcdf_gbx` (0.4.1) | `netcdf_gdal` / `netcdf_ogr` (0.4.3) |
| [PMTiles](./readers/pmtiles) | `pmtiles_gbx` (0.4.0) | — (light-only) |
| [File lister](./readers/file) | `file_gbx` (0.5.0) | — (light-only) |
| [LiDAR (LAS/LAZ)](./readers/lidar) | `lidar_gbx` (0.5.2) | — (light-only) |
| [EXIF/GPS (images)](./readers/exif) | `exif_gbx` (0.5.2) | — (light-only) |
| [Vector (generic)](./readers/vector) | `vector_gbx` (0.4.0) | `ogr` (0.2.0) |
| [Shapefile](./readers/shapefile) | `shapefile_gbx` (0.4.0) | `shapefile_ogr` (0.2.0) |
| [GeoJSON](./readers/geojson) | `geojson_gbx` (0.4.0) | `geojson_ogr` (0.2.0) |
| [GeoJSONL](./writers/geojsonl) | `geojsonl_gbx` (0.4.0) | — (light-only reader) |
| [GeoPackage](./readers/geopackage) | `gpkg_gbx` (0.4.0) | `gpkg_ogr` (0.2.0) |
| [File Geodatabase](./readers/filegdb) | `file_gdb_gbx` (0.4.0) | `file_gdb_ogr` (0.2.0) |
...
| Format | Lightweight | Heavyweight |
|---|---|---|
| [Raster (generic)](./writers/raster) | `raster_gbx` (0.4.0) | `gdal` (0.2.0) |
| [GeoTIFF](./writers/geotiff) | `gtiff_gbx` (0.4.0) | `gtiff_gdal` (0.2.0) |
| [COG](./writers/cog) | `cog_gbx` (0.5.0) | — (light-only) |
| [PMTiles](./writers/pmtiles) | `pmtiles_gbx` (0.4.0) | `pmtiles` (0.4.0) |
| [NetCDF](./writers/netcdf) | `netcdf_gbx` (0.4.1) | — (light-only) |
| [Vector (generic)](./writers/vector) | `vector_gbx` (0.4.0) | — (light-only) |
| [Shapefile](./writers/shapefile) | `shapefile_gbx` (0.4.0) | — (light-only) |
| [GeoJSON](./writers/geojson) | `geojson_gbx` (0.4.0) | — (light-only) |
| [GeoJSONL](./writers/geojsonl) | `geojsonl_gbx` (0.4.0) | `geojsonl_ogr` (0.4.0) |
| [GeoPackage](./writers/geopackage) | `gpkg_gbx` (0.4.0) | — (light-only) |
| [File Geodatabase](./writers/filegdb) | `file_gdb_gbx` (0.4.0) | — (hybrid; needs native GDAL) |
...
- **FILE-capable runtime (DBR 13.3+):** routes to the FILE API. Files at or under
  `GBX_STREAM_MAX_BYTES` (default **64 MiB on Serverless/Connect**, 256 MiB on classic)
  open a byte-range **stream** — one round-trip, no FUSE, no local copy, and the fastest
  path for typical tile sizes. Files over the cap fall through to `as_local_file()` — a
  lazy local-file reference backed by the FILE handle — which pages bytes from the Volume
  on demand and is a **memory-safety mechanism, not a performance optimization**. A blanket
  FUSE-direct path for small files is slower, not faster; the streaming path is preferred
  whenever the file fits in the cap. Open dataset handles are held in a per-partition LRU
...
- [VRT & Mosaics](./api/vrt-mosaic) — tile a large source into bounded mini-COGs + a portable VRT index; read back with `raster_gbx` or `cog_gbx`.
- [Serverless & Memory](./serverless-and-memory) — connect-aware caps, materialize-vs-virtual, memory-safe write patterns, and caveats.
