...
by the [`cog_gbx` writer's mosaic mode](../writers/cog#vrt-mosaic-mode)) and the
...
[Reading a VRT mosaic](#reading-a-vrt-mosaic) below and the
[VRT Mosaics](../api/vrt-mosaic) reference.
...
When the load path points **directly at a `mosaic.vrt`** (written by the
[`cog_gbx` writer's mosaic mode](../writers/cog#vrt-mosaic-mode)), the reader
...
[VRT Mosaics](../api/vrt-mosaic) page.
