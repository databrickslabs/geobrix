...
|--------|---------|-------------|
| `splitStrategy` | `"auto"` | When to split large rasters: `auto` (default, budget-based), `serverless`, `classic`, `none` (no split). See [Raster Options](./raster#options). |
| `tileFormat` | `"auto"` | Output format: `auto` (passthrough when no split; COG when splitting), `cog`, `gtiff`. |
| `cogBlockSize` | `"512"` | Internal tile size (px) for COG output. |
| `cogOverviewResampling` | `"AVERAGE"` | Resampling algorithm for COG overviews. |
