...
| `enumerate_files` (FILE-tier enumeration) | Used by the function layer | The `spark.read.format` DataSource is session-less on Connect; FILE-tier enumeration is only available through `gbx_file_read` at the function layer. |
