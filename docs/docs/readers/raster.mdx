...
_Since:_ lightweight `raster_gbx` **v0.4.0** · heavyweight `gdal` **v0.2.0**
...
| `tilesTable` | _none_ | Delta table to use as a tile index (see [pre-computed tile inputs](#advanced-pre-computed-tile-inputs) below). |
| `skipOrdering` | `"false"` | Pass `"true"` to suppress the auto-sort-by-source-path that `tilesTable` reads apply by default. See [tilesTable auto-order](#tiles-table-auto-order) below. |
...
#### Directory reads {#loading-many-small-files}

Reading a directory plans every file (fast, stat-free listing — ~1.5 s at 10k files); for pre-computed windows or very large tile counts see [Advanced: pre-computed tile inputs](#advanced-pre-computed-tile-inputs) below.
...
## Advanced: pre-computed tile inputs (`manifest` / tile index) {#advanced-pre-computed-tile-inputs}
...
Both options reduce plan time from the directory walk (~1.5 s at 10k files) to a single file or table read. See [Benchmarking → Reader plan-time listing](../api/benchmarking#reader-plan-time-listing) for the measured numbers.
...
### tilesTable auto-order and `skipOrdering` {#tiles-table-auto-order}
