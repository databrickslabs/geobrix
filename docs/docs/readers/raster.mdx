...
| `sizeInMB` | `"-1"` | Power-user override: set a positive value to pin the per-tile decoded-memory budget in MiB, overriding the strategy budget. `sizeInMB=-1` (the default) means defer to the active `splitStrategy` budget — this is why large rasters auto-split by default with `splitStrategy=auto`. Ignored when `splitStrategy=none`. |
