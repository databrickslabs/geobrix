...
|--------|---------|-------------|
| `splitStrategy` | `"none"` | When to split large rasters: `none` (default — one whole-image tile per file), `auto` (resolve to `serverless` or `classic` by environment), `serverless` (opt-in, 128 MiB decoded budget per tile), `classic` (opt-in, 1 536 MiB decoded budget per tile). |
