...
| `clipPolygons` | _none_ | Area(s) of interest to clip to. One WKT/EWKT string for a single polygon, or a JSON-array string for a list, e.g. `'["POLYGON((...))","POLYGON((...))"]'` (raw WKB/EWKB bytes are accepted only from programmatic callers). One tile is emitted per polygon whose envelope intersects the raster; a polygon that misses the raster emits no tile. Materialized tiles are pre-clipped (pixels outside the polygon are NoData); virtual tiles carry the clip as a deferred instruction. Mutually exclusive with `windows`. Pairs naturally with the [COG Reader](./cog). |
| `windows` | _none_ | Pixel window(s) to read. A JSON 4-int array `"[col,row,w,h]"` for one, or a JSON array of them `"[[..],[..]]"` for a list. One tile per window; partial windows clip to the raster extent, fully-outside windows are skipped. Mutually exclusive with `clipPolygons`. |
