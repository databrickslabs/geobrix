...
- **New `lidar_gbx` Spark DataSource** reads LAS and LAZ point-cloud files into a
  ...
  number), `decimate` (keep every N-th point), `chunkSize`. Lightweight only;
  Serverless-safe. See [LiDAR Reader](./readers/lidar).
...
- **`gbx_quadbin_geomkring` / `gbx_quadbin_geomkloop`, `gbx_bng_geomkring` /
  ...
  `gbx_h3_geomkring` / `gbx_h3_geomkloop` also move to a boundary-ring seed path
  in this release, with a correctness improvement: the implementation now uses H3-native
  per-cell containment tests so that results on grid-aligned polygons (polygon edges that
  lie exactly on H3 cell boundaries) are byte-identical to H3's own polyfill oracle.
  For large H3 polygons at fine resolutions these per-cell tests can make the H3 path
  slower than prior versions; for small and medium polygons the performance is similar.
