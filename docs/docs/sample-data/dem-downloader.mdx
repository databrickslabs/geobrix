...
For **true 1 m** (or finer, e.g. to match H3 resolution 13–15), the raster catalogs are not enough — derive elevation from the raw LiDAR point cloud via `LidarDownloader` + `rst_binpoints_agg`. See the [LiDAR reader](../readers/lidar) and the [Wireless Coverage](../notebooks/wireless-coverage) notebook series.
