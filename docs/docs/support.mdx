...
3. **Check Known Limitations**: Review [Known Limitations](./limitations) to see if it's a documented limitation
...
- Databricks Runtime version (e.g., DBR 17.3 LTS, 18 LTS, or 19 LTS)
...
<CodeFromTest language="markdown" validationLevel="static" title="Issue Template Example">
{`**Environment:**
- DBR: 17.3 LTS (or 18 LTS or 19 LTS)
...
<CodeFromTest language="java" validationLevel="static" title="Example Error">
{`java.lang.OutOfMemoryError: Java heap space`}
</CodeFromTest>

<CodeFromTest language="python" validationLevel="static" title="Reproduction Code">
{`df = spark.read.format("gdal").load("/data/large.tif")
df.count()`}
</CodeFromTest>

<CodeFromTest language="markdown" validationLevel="static" title="Expected vs Actual">
{`**Expected:**
...
(The `gdal` reader shown above is part of the heavyweight tier — see [Choosing an Execution Tier](./api/execution-tiers).)

### Reproduce on Databricks Runtime 19

On a Databricks Runtime 19 cluster (light tier):

<CodeFromTest language="python" validationLevel="static" title="DBR 19 Reproduction Template">
{`# Adapt the path below to your staged wheel location (see installation guide).
