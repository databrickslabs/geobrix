...
_Since:_ lightweight `shapefile_gbx` **v0.4.0** · _(light-only — no heavyweight tier)_
...
| `gbx_file_write` / FILE-tier write | Not in the DataSource | The DataSource writer (`df.write.format("shapefile_gbx")`) is session-less on Connect and commits via FUSE. FILE-tier writes use `vector_file_write` at the function layer instead. |
