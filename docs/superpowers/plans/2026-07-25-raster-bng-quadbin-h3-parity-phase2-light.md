...
- **Light-CI checklist:** no new deps (pygx/rasterio/shapely/pyproj/numpy already present); light test dirs `pyrx`/`pygx` already in `_LIGHT_TEST_DIRS` (new tests land under `python/geobrix/test/pyrx/`); no lockfile change expected — if any dep is added, pin across all 3 envs + recompile hashed txt.
