...
- **Tab + output consistency is a per-function review gate:** (a) all present tabs show the SAME example — same fixture, operation, and argument values, differing only in language; (b) output renders CONSISTENTLY — a value identical across tiers is shown identically (same truncation + same format annotation), and a value that legitimately differs per tier is LABELED as a real divergence (never fabricate or hide one). A reader must never wonder whether a tab difference is a real tier difference, a different chosen example, or an inaccurate doc. The `docs-examples` guard enforces (b)'s annotation consistency + ASCII-table width.
...
- **CRS-family SQL returns BINARY** (memory `vectorx-crs-family-decisions`).
- **Uniform output representation (VectorX is geometry-dominated — this is a hard rule, applied identically across every tab of a function AND consistently page-wide; enforced by the `docs-examples` guard's ASCII-width + annotation-consistency checks).** Never dump raw bytes or a huge string into an output table. ONE elision token — three ASCII dots `...` (matching the existing RasterX `...`+`(WKB binary)` convention) — plus a parenthetical FORMAT annotation:
  - **Binary geometry** (`[E]WKB` returns — `st_setcrs`, `st_transformcrs`, `st_legacyaswkb`, `st_triangulate` when WKB): `...` + `(WKB binary)` / `(EWKB binary)`. **MVT bytes** (`st_asmvt`, `st_asmvt_pyramid`): `...` + `(MVT binary)`.
  - **`[E]WKT`**: show in FULL when short (e.g. `POINT (15 51)`, a small polygon); truncate long ones keeping the geometry TYPE + leading coordinate(s), eliding the interior with `...`, e.g. `POLYGON ((530000 180000, 530100 180000, ...))` + `(WKT)` / `(EWKT)`.
  - **GeoJSON**: truncated structural form `{"type":"Polygon","coordinates":[[...]]}` + `(GeoJSON)`.
  - **CRS strings** (`st_crs`): short authority in full (`EPSG:4326`, `OGC:CRS84`); long WKT CRS truncated `PROJCS["WGS 84 / UTM zone 33N", ...]`.
  - **Cell width cap ~60 chars** so the ASCII table stays width-consistent. Tabs whose output is byte-identical carry the SAME annotation (tier-specific `; light tier ...` clauses excepted).
...
- [ ] **Step 4:** Add the Setup + Conventions section to `vectorx-functions.mdx`, copying the RasterX template shape (placeholder→view→backs table; "assumes library installed" linking installation.mdx; tab-reading conventions; CRS-family BINARY note). **State the uniform output-representation convention ONCE here** (from Global Constraints: the `...`+format-annotation scheme for `[E]WKB`/MVT binary, `[E]WKT`, GeoJSON, CRS strings; the ~60-char cell cap) so every per-function example just follows it. Every batch's review (and `gbx:test:docs-examples`) checks each function's output against this convention.
