...
## 7b. At-scale reader benchmark (comparative)
Reuse the existing `gbx:bench:readers` (`bench.readers`): spark-path mode runs `raster_gbx.load(dir).count()`, which is **planning-dominated** — `.count()` triggers the full `partitions()` listing + per-file header-open rake but reads no pixels — so it measures the listing cost directly. Per the eo-series scale + the virtual-tile caveat, the comparative is:
- **Scale (reproduce the rake):** generate a HIGH-COUNT **tiny-tile** corpus (the rake is file-COUNT-driven, not size): `gbx:bench:gen-data --row-rows 27000 --tile-px 256 --bands 1 --dtypes float32` (~256 KB/tile → manageable; 10K to bound cost). Listing/planning is DRIVER-bound, so this leg doesn't need 20 workers — a small cluster suffices.
- **Listing comparative (Thread B fix):** `gbx:bench:readers --mode spark-path` on that corpus — baseline (`.load(dir)` walk + per-file header open) vs the fix (`manifest`/`tilesTable` input, and lazy-planning passthrough). `--run-id` is only a label (runs use the deployed code), so bracket the change: baseline on today's reader vs fixed via the new `manifest` option / post-fix build. Compare `iter_median_s` + `throughput_rows_s`; expect the per-file header rake to collapse to one manifest read / deferred header.
- **Isolate plan vs read (rigor):** the reader bench does NOT currently split `.load()` planning from `.count()` execution — add lightweight instrumentation (or a flag) to `run_spark_path_reader` to time them separately, so the planning-time drop is measured directly, not just inferred from the (planning-dominated) total.
- **Virtual-tile read, FILE-on vs FILE-off (the caveat — readers were never benchmarked for virtual tiles):** `.count()` doesn't read pixels, so it never exercises FILE. Add a **pixel-reading** leg over the reader's VIRTUAL tiles (e.g. an `rst_summary`/stat aggregate over `.load(dir)`), run **FILE-ON vs FILE-OFF** (`GBX_DISABLE_FILE=1`) on the 20-worker DBR 19 dedicated cluster, to establish the virtual-tile reader read baseline + the FILE byte-range win in the reader path. This is the comparison the readers lack today (and complements Thread A's per-op A/B).
