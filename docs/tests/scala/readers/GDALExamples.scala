...
+--------------------------------------------------+-----+"""
  ...
  def readGDAL(spark: SparkSession, path: String = SampleDataPath.nycSentinel2): DataFrame = {
    spark.read.format("gdal").load(path)
  }

  def readWithDriver(spark: SparkSession, path: String = SampleDataPath.nycSentinel2): DataFrame = {
