...
- **GeoBrix 0.5.0.** `config_nb.ipynb` `%pip`-installs `geobrix[light_env6,stac,vizx,earthdata]` from a staged Volume wheel using the 2-step pattern (force-reinstall `--no-deps` first, then with the extras to pick up dependencies). For the heavyweight option, flip *option-2* in `config_nb.ipynb` and attach the GeoBrix JAR + GDAL init script to the cluster.
