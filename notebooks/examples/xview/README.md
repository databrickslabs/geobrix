...
- **GeoBrix** (version 0.5.0). The setup cell `%pip`-installs the `geobrix[light_env6,vizx]` wheel: `[light_env6]` brings the pure-Python bindings (`databricks.labs.gbx.pyrx`) and their dependencies, and `[vizx]` brings the [VizX](https://databrickslabs.github.io/geobrix/docs/api/vizx) preview helpers (`plot_raster` / `plot_file`) used for the in-notebook previews. For the heavyweight tweak, also attach the GeoBrix JAR **and the GDAL init script** to the cluster, and switch the import to `databricks.labs.gbx.rasterx`.
