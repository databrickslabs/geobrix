"""Task 8: reader layout conventions — path-ordered emission + numPartitions option.

Verifies:
- Both readers accept ``numPartitions`` (and ``repartition`` as alias).
- Invalid values raise ValueError.
- Default is None (option absent).
- Raster manifest path: partitions are sorted by file_path.
- Raster default (os.walk) path: partitions are already path-ordered (list_files sorted).
- Vector default path: partitions are path-ordered via sorted _members().
"""

from __future__ import annotations

import json
from unittest.mock import patch

import pytest

from databricks.labs.gbx.ds.raster import RasterGbxReader, _TilePartition
from databricks.labs.gbx.ds.vector import VectorGbxReader

# ---------------------------------------------------------------------------
# Raster: numPartitions option parsing
# ---------------------------------------------------------------------------


def test_raster_num_partitions_parsed():
    r = RasterGbxReader({"path": "/x", "numPartitions": "4"})
    assert r.num_partitions == 4


def test_raster_repartition_alias():
    r = RasterGbxReader({"path": "/x", "repartition": "8"})
    assert r.num_partitions == 8


def test_raster_num_partitions_default_none():
    r = RasterGbxReader({"path": "/x"})
    assert r.num_partitions is None


def test_raster_num_partitions_invalid_string_raises():
    with pytest.raises(ValueError, match="numPartitions"):
        RasterGbxReader({"path": "/x", "numPartitions": "abc"})


def test_raster_num_partitions_zero_raises():
    with pytest.raises(ValueError, match="numPartitions"):
        RasterGbxReader({"path": "/x", "numPartitions": "0"})


def test_raster_num_partitions_negative_raises():
    with pytest.raises(ValueError, match="numPartitions"):
        RasterGbxReader({"path": "/x", "numPartitions": "-1"})


# ---------------------------------------------------------------------------
# Raster: path-ordered partition emission
# ---------------------------------------------------------------------------


def test_raster_manifest_partitions_path_ordered(tmp_path):
    """Manifest rows with paths in reverse-alpha order → partitions sorted by file_path."""
    manifest = [
        {"path": "/z/z.tif", "window": [0, 0, 4, 4]},
        {"path": "/a/a.tif", "window": [0, 0, 4, 4]},
        {"path": "/m/m.tif", "window": [0, 0, 4, 4]},
    ]
    mf = str(tmp_path / "tiles.json")
    with open(mf, "w") as fh:
        json.dump(manifest, fh)

    r = RasterGbxReader({"path": "/x", "manifest": mf})
    parts = r.partitions()
    paths = [p.file_path for p in parts]
    assert paths == sorted(paths), f"Expected sorted by file_path, got {paths}"


def test_raster_manifest_multi_tile_per_file_grouped(tmp_path):
    """Multiple tiles per file in manifest → tiles from same file are adjacent after sort."""
    manifest = [
        {"path": "/z/z.tif", "window": [0, 0, 4, 4]},
        {"path": "/a/a.tif", "window": [0, 0, 4, 4]},
        {
            "path": "/z/z.tif",
            "window": [4, 0, 4, 4],
        },  # second tile of z.tif, listed last
        {"path": "/a/a.tif", "window": [4, 0, 4, 4]},
    ]
    mf = str(tmp_path / "tiles.json")
    with open(mf, "w") as fh:
        json.dump(manifest, fh)

    r = RasterGbxReader({"path": "/x", "manifest": mf})
    parts = r.partitions()
    paths = [p.file_path for p in parts]
    # After sort: a.tif, a.tif, z.tif, z.tif — same-source tiles are adjacent
    assert paths == sorted(paths), f"Expected sorted by file_path, got {paths}"


def test_raster_default_path_partitions_path_ordered():
    """Default os.walk path: list_files returns sorted paths → partitions are path-ordered."""
    with (
        patch("databricks.labs.gbx.ds.raster._listing.list_files") as mock_lf,
        patch("databricks.labs.gbx.ds.raster._plan_partitions_for_file") as mock_ppf,
    ):
        mock_lf.return_value = ["/a/a.tif", "/b/b.tif", "/c/c.tif"]
        mock_ppf.side_effect = lambda file_path, **kw: [
            _TilePartition(file_path=file_path, window=(0, 0, 4, 4))
        ]
        r = RasterGbxReader({"path": "/some/dir"})
        parts = r.partitions()
    paths = [p.file_path for p in parts]
    assert paths == sorted(paths), f"Expected sorted by file_path, got {paths}"


# ---------------------------------------------------------------------------
# Vector: numPartitions option parsing
# ---------------------------------------------------------------------------


def test_vector_num_partitions_parsed():
    r = VectorGbxReader({"path": "/x/a.geojson", "numPartitions": "4"})
    assert r.num_partitions == 4


def test_vector_repartition_alias():
    r = VectorGbxReader({"path": "/x/a.geojson", "repartition": "8"})
    assert r.num_partitions == 8


def test_vector_num_partitions_default_none():
    r = VectorGbxReader({"path": "/x/a.geojson"})
    assert r.num_partitions is None


def test_vector_num_partitions_invalid_string_raises():
    with pytest.raises(ValueError, match="numPartitions"):
        VectorGbxReader({"path": "/x/a.geojson", "numPartitions": "abc"})


def test_vector_num_partitions_zero_raises():
    with pytest.raises(ValueError, match="numPartitions"):
        VectorGbxReader({"path": "/x/a.geojson", "numPartitions": "0"})


def test_vector_num_partitions_negative_raises():
    with pytest.raises(ValueError, match="numPartitions"):
        VectorGbxReader({"path": "/x/a.geojson", "numPartitions": "-1"})


# ---------------------------------------------------------------------------
# Vector: path-ordered partition emission
# ---------------------------------------------------------------------------


def test_vector_default_partitions_path_ordered(tmp_path):
    """Multiple .geojson files → partitions are emitted in sorted path order."""
    geojson_content = json.dumps({"type": "FeatureCollection", "features": []})
    # Write in deliberately non-alpha order; _members() must sort
    for name in ("z_file.geojson", "a_file.geojson", "m_file.geojson"):
        (tmp_path / name).write_text(geojson_content)

    r = VectorGbxReader({"path": str(tmp_path), "driverName": "GeoJSON"})
    parts = r.partitions()
    paths = [p.path for p in parts]
    assert paths == sorted(paths), f"Expected sorted paths, got {paths}"
