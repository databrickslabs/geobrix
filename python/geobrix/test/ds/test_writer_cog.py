...
def test_writer_cog_true_converts_plain_gtiff(tmp_path):
    w = RasterGbxWriter(
        str(tmp_path), reader_schema(), overwrite=True, cog=True, cog_blocksize=256
    )
    b = _plain_gtiff()
    row = {
        "source": "x",
        "tile": {"cellid": -1, "raster": b, "metadata": {"driver": "GTiff"}},
    }
    w.write(iter([row]))
    import glob
    import os
...
def test_writer_cog_passthrough_already_cog(tmp_path):
    ...
    row = {
        "source": "x",
        "tile": {"cellid": -1, "raster": cb, "metadata": {cog.GBX_FORMAT: "cog"}},
    }
    w.write(iter([row]))
    import glob
    import os
