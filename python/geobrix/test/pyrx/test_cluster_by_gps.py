...
def _grid_df(
    n_side: int, spacing_deg: float = 1e-4, lat0: float = 37.75, lon0: float = -122.45
):
...
def test_overlap_duplicates_boundary_images():
    ...
    assert (
        per_image >= 2
    ).any(), "overlap_frac>0 should share some images across clusters"
...
def test_zero_overlap_is_a_partition():
    df = _grid_df(12)
    out = cluster_by_gps(
        df, target_cluster_images=30, overlap_frac=0.0, min_cluster_images=1
    )
    per_image = out.groupby("source")["_cluster"].nunique()
