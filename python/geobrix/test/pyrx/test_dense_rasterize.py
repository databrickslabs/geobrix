...
def test_zbuffer_ortho_top_surface_rgb_and_dsm():
    ...
    x = np.array([0.5, 0.5, 10.5]); y = np.array([0.5, 0.5, 0.5]); z = np.array([1.0, 5.0, 2.0])
    r = np.array([10, 200, 30], "u1"); g = np.array([10, 200, 30], "u1"); b = np.array([10, 200, 30], "u1")
    rgb, dsm, transform = zbuffer_ortho(x, y, z, r, g, b, xmin=0, ymin=0, xmax=11, ymax=1, px=1.0)
    assert rgb.shape[0] == 3, "RGB is band-first (3,H,W)"
    assert tuple(int(v) for v in rgb[:, 0, 0]) == (200, 200, 200)  # higher-z point's color wins cell (0,0)
...
def test_zbuffer_ortho_empty_cells_are_nodata():
    x = np.array([0.5]); y = np.array([0.5]); z = np.array([3.0])
    r = g = b = np.array([100], "u1")
    rgb, dsm, transform = zbuffer_ortho(x, y, z, r, g, b, xmin=0, ymin=0, xmax=3, ymax=1, px=1.0)
