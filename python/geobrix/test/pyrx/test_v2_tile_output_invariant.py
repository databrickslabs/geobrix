...
2. ``test_no_legacy_tile_schema_in_source`` — a static guard that greps the pyrx
   source for ANY reference to the deleted legacy ``TILE_SCHEMA`` constant, in ALL
   THREE declaration forms (``@f.udf(_serde.TILE_SCHEMA)`` decorator,
   ``f.udf(_serde.TILE_SCHEMA)(fn)`` functional, ``@udtf(returnType=_serde.TILE_SCHEMA)``).
   The registry-only guard (#1) cannot see the INTERNAL functional-form UDF
   (``_as_tile_cellid_envelope_udf``) because it is not registered — this static
   guard catches a regression there too, and any future reintroduction of the
   legacy 3-field output.
...
def test_no_legacy_tile_schema_in_source():
    """No pyrx source references the deleted legacy TILE_SCHEMA constant, in ANY
    declaration form. Catches the internal (unregistered) functional-form UDF that
    the registry guard above cannot see, and any future reintroduction."""
    src_dir = Path(fns.__file__).parent
    # match _serde.TILE_SCHEMA but NOT V2_TILE_SCHEMA (negative lookbehind on 'V2_')
    pat = re.compile(r"(?<!V2_)\bTILE_SCHEMA\b")
    offenders = []
    ...
    assert not offenders, (
        "Legacy TILE_SCHEMA reference(s) survive in pyrx source (must be "
        "V2_TILE_SCHEMA everywhere):\n" + "\n".join(offenders)
    )
