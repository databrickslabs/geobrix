...
NETCDF = (HERE.parents[4] / "src/test/resources/binary/netcdf-CMIP5/prAdjust_day_HadGEM2-CC_SMHI-DBSrev930-GFD-1981-2010-postproc_rcp45_r1i1p1_20201201-20201231.nc").resolve()
...
def test_rst_min_max(spark):
    ...
    df = spark.range(1).select(
        rx.rst_min(tile_col).alias("min_value"),
        rx.rst_max(tile_col).alias("max_value")
    )
...
def test_rst_bands(spark):
    ...
    df = spark.range(1).select(
        rx.rst_numbands(tile_col).alias("num_bands")
    )
...
def test_rst_nodata(spark):
    ...
    df = spark.range(1).select(
        rx.rst_getnodata(tile_col).alias("nodata_values")
    )
...
def test_rst_boundingbox(spark):
    ...
    df = spark.range(1).select(
        rx.rst_boundingbox(tile_col).alias("bbox")
    )
...
def test_rst_memsize(spark):
    ...
    df = spark.range(1).select(
        rx.rst_memsize(tile_col).alias("mem_size")
    )
...
def test_rst_summary(spark):
    ...
    df = spark.range(1).select(
        rx.rst_summary(tile_col).alias("summary")
    )
...
def test_rst_subdatasets(spark):
    ...
    df = spark.range(1).select(
        rx.rst_subdatasets(tile_col).alias("subdatasets")
    )
...
def test_all_accessors_together(spark):
    ...
    assert all(key in row_dict for key in ["avg", "min", "max", "width", "height", "bands", "format", "srid"])
