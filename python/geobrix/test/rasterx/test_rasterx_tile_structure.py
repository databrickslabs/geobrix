...
def test_tile_from_content_contains_binary(spark):
    ...
    df = spark.read.format("binaryFile").load(str(MODIS_DIR)).select(
...
def test_tessellated_tiles_have_cellid(spark):
    ...
    df = spark.range(1).select(
        ...
        rx.rst_h3_tessellate(f.col("tile"), f.lit(1)).alias("tess_tile")
    ).select(
        f.col("tess_tile.cellid").alias("cellid")
    ).limit(10)  # Limit for test efficiency
...
def test_extract_raster_path_for_conditional_processing(spark):
    ...
    assert result[0]["is_b01"] == True
    assert result[1]["is_b01"] == False
...
def test_access_raster_binary_for_custom_udfs(spark):
    ...
    df = spark.read.format("binaryFile").load(str(MODIS_DIR)).limit(1).select(
        rx.rst_fromcontent(f.col("content"), f.lit("GTiff")).alias("tile")
    ).select(
        f.col("tile.raster").alias("raster_binary"),
        f.col("tile.metadata").alias("metadata")
