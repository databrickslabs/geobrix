...
USAGE:
    bash scripts/commands/gbx-docs-static-build.sh [OPTIONS]
...
EXAMPLES:
    ...
    bash scripts/commands/gbx-docs-static-build.sh

    # Zip to a custom folder (zip name still uses version from docs/package.json)
    bash scripts/commands/gbx-docs-static-build.sh --output ./docs-build

    # Build only (no zip)
    bash scripts/commands/gbx-docs-static-build.sh --skip-zip
