...
/** Expression that returns the Euclidean distance between two BNG cell centres (metres). Arguments: cellId, cellId2. */
...
object BNG_EuclideanDistance extends WithExpressionInfo {
    ...
    def execute(cellId: Long, cellId2: Long): Long = BNG.euclideanDistance(cellId, cellId2)

    def execute(cellId: String, cellId2: String): Long = {
        val cellIdLong = BNG.parse(cellId)
        val cellId2Long = BNG.parse(cellId2)
        BNG.euclideanDistance(cellIdLong, cellId2Long)
    }
