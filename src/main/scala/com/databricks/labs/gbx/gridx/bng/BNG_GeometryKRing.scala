...
/** Expression that returns the k-ring geometry (polygon) for a BNG cell at resolution.
...
/** Companion: SQL name gbx_bng_geomkring, builder, and eval. */
object BNG_GeometryKRing extends WithExpressionInfo {
    ...
    def execute(geom: Geometry, res: Int, k: Int, mode: String, coverage: String): Set[String] = {
        val kRing = BNG.geometryKRing(geom, res, k, mode, coverage)
    ...
    override def builder(): FunctionBuilder = (c: Seq[Expression]) => c.length match {
        case 3 => new BNG_GeometryKRing(c(0), c(1), c(2), Literal(GeomDilation.DEFAULT_MODE), Literal(GeomDilation.DEFAULT_COVERAGE))
        case 4 => new BNG_GeometryKRing(c(0), c(1), c(2), c(3), Literal(GeomDilation.DEFAULT_COVERAGE))
        case 5 => new BNG_GeometryKRing(c(0), c(1), c(2), c(3), c(4))
    }
