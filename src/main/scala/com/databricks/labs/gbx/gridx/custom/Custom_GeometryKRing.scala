...
/** Catalyst expression: geometry-aware k-ring for a custom-grid cell set.
  *
  * Returns the set of cell IDs reachable from the geometry's covering set in at most k
  * Chebyshev steps, controlled by a dilation mode.
  *
  ...
  */
case class Custom_GeometryKRing(
    geomExpr:       Expression,
    gridExpr:       Expression,
    ...
    override def eval(input: InternalRow): Any = {
        ...
        GridErrorHandler.safeEval[ArrayData](null) {
            ...
            ArrayData.toArrayData(sys.geometryKRing(geom, res, k, mode, coverage).toArray)
        }
    }
...
/** Companion: SQL name gbx_custom_geomkring, 4-/5-/6-arg builder. */
object Custom_GeometryKRing extends WithExpressionInfo {

    override def name: String = "gbx_custom_geomkring"

    override def builder(): FunctionBuilder = (c: Seq[Expression]) => c.length match {
        case 4 => new Custom_GeometryKRing(c(0), c(1), c(2), c(3), Literal(GeomDilation.DEFAULT_MODE), Literal(GeomDilation.DEFAULT_COVERAGE))
        case 5 => new Custom_GeometryKRing(c(0), c(1), c(2), c(3), c(4), Literal(GeomDilation.DEFAULT_COVERAGE))
        case 6 => new Custom_GeometryKRing(c(0), c(1), c(2), c(3), c(4), c(5))
        case n => throw new IllegalArgumentException(
            s"gbx_custom_geomkring requires 4, 5, or 6 arguments; got $n")
    }
