...
/** Generator that explodes the geometry-aware k-loop (hollow ring) for quadbin into one row per cell.
  * Arguments: geom, resolution, k[, mode]. mode is optional (default: boundary-out).
  * Yields rows with a single BIGINT cell id. */
case class Quadbin_GeometryKLoopExplode(
    geom: Expression,
    ...
    override def eval(input: InternalRow): IterableOnce[InternalRow] = {
        ...
        if (geometryRaw == null || resolutionRaw == null || kRaw == null) {
            ...
            GridErrorHandler.safeEval[IterableOnce[InternalRow]](Iterator.empty) {
                ...
                Quadbin.geometryKLoop(geometryVal, resolutionVal, kVal, modeStr, coverageStr)
...
/** Companion: SQL name gbx_quadbin_geomkloopexplode, builder. */
object Quadbin_GeometryKLoopExplode extends WithExpressionInfo {

    override def name: String = "gbx_quadbin_geomkloopexplode"

    override def builder(): FunctionBuilder = (c: Seq[Expression]) => c.length match {
        case 3 => new Quadbin_GeometryKLoopExplode(c(0), c(1), c(2), Literal(GeomDilation.DEFAULT_MODE), Literal(GeomDilation.DEFAULT_COVERAGE))
        case 4 => new Quadbin_GeometryKLoopExplode(c(0), c(1), c(2), c(3), Literal(GeomDilation.DEFAULT_COVERAGE))
        case 5 => new Quadbin_GeometryKLoopExplode(c(0), c(1), c(2), c(3), c(4))
    }
