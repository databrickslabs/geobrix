...
case class RST_Slope(
    ...
    override def children: Seq[Expression] = Seq(tile, unitExpr, scaleExpr, ExpressionConfigExpr())
    // Pin types so SQL decimal literals (e.g. ``1.0``) coerce to Double cleanly.
    override def inputTypes: Seq[DataType] = Seq(tile.dataType, StringType, DoubleType, StringType)
