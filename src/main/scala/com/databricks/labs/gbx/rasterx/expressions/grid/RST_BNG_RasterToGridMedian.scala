...
case class RST_BNG_RasterToGridMedian(tileExpr: Expression, resolution: Expression) extends InvokedExpression {
    ...
    override def replacement: Expression = rstInvoke(RST_BNG_RasterToGridMedian, rasterType)
...
object RST_BNG_RasterToGridMedian extends WithExpressionInfo {
    ...
    def execute(ds: Dataset, resolution: Int): Array[Array[(String, Double)]] = {
        ...
        RST_BNG_RasterToGrid.execute(ds, resolution, medianF)
    }

    override def name: String = "gbx_rst_bng_rastertogridmedian"

    override def builder(): FunctionBuilder = (c: Seq[Expression]) => new RST_BNG_RasterToGridMedian(c(0), c(1))
