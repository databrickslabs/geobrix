...
case class RST_BNG_RasterToGridVariance(tileExpr: Expression, resolution: Expression) extends InvokedExpression {
    ...
    override def prettyName: String = RST_BNG_RasterToGridVariance.name
    override def replacement: Expression = rstInvoke(RST_BNG_RasterToGridVariance, rasterType)
...
object RST_BNG_RasterToGridVariance extends WithExpressionInfo {
    ...
    def execute(ds: Dataset, resolution: Int): Array[Array[(String, Double)]] = {
        ...
        RST_BNG_RasterToGrid.execute(ds, resolution, varF)
    }

    override def name: String = "gbx_rst_bng_rastertogridvariance"

    override def builder(): FunctionBuilder = (c: Seq[Expression]) => new RST_BNG_RasterToGridVariance(c(0), c(1))
