...
case class RST_H3_RasterToGridStddev(
    tileExpr: Expression,
    ...
    override def prettyName: String = RST_H3_RasterToGridStddev.name
    override def replacement: Expression = rstInvoke(RST_H3_RasterToGridStddev, rasterType)
    override protected def withNewChildrenInternal(nc: IndexedSeq[Expression]): Expression = copy(nc(0), nc(1))

}

/** Companion: SQL name, builder, and eval entry points for path/binary tile. */
object RST_H3_RasterToGridStddev extends WithExpressionInfo {
    ...
    def execute(ds: Dataset, resolution: Int): Array[Array[(Long, Double)]] = {
        ...
        RST_H3_RasterToGrid.execute(ds, resolution, stdF)
    }

    override def name: String = "gbx_rst_h3_rastertogridstddev"

    override def builder(): FunctionBuilder = (c: Seq[Expression]) => new RST_H3_RasterToGridStddev(c(0), c(1))
