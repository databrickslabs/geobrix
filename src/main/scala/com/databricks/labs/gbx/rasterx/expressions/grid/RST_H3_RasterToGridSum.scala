...
case class RST_H3_RasterToGridSum(
    tileExpr: Expression,
    ...
    override def prettyName: String = RST_H3_RasterToGridSum.name
    override def replacement: Expression = rstInvoke(RST_H3_RasterToGridSum, rasterType)
    override protected def withNewChildrenInternal(nc: IndexedSeq[Expression]): Expression = copy(nc(0), nc(1))

}

/** Companion: SQL name, builder, and eval entry points for path/binary tile. */
object RST_H3_RasterToGridSum extends WithExpressionInfo {
    ...
    def execute(ds: Dataset, resolution: Int): Array[Array[(Long, Double)]] = {
        ...
        RST_H3_RasterToGrid.execute(ds, resolution, sumF)
    }

    override def name: String = "gbx_rst_h3_rastertogridsum"

    override def builder(): FunctionBuilder = (c: Seq[Expression]) => new RST_H3_RasterToGridSum(c(0), c(1))
