...
/** Returns the average raster value within each quadbin grid cell. */
case class RST_Quadbin_RasterToGridAvg(
    tileExpr: Expression,
    ...
    override def prettyName: String = RST_Quadbin_RasterToGridAvg.name
    override def replacement: Expression = invoke(RST_Quadbin_RasterToGridAvg)
...
object RST_Quadbin_RasterToGridAvg extends WithExpressionInfo {
    ...
    def execute(ds: Dataset, resolution: Int): Array[Array[(Long, Double)]] = {
        ...
        RST_Quadbin_RasterToGrid.execute(ds, resolution, meanF)
    }

    override def name: String = "gbx_rst_quadbin_rastertogridavg"

    override def builder(): FunctionBuilder = (c: Seq[Expression]) => new RST_Quadbin_RasterToGridAvg(c(0), c(1))
