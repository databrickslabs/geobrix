...
/** Returns the sum of the raster values within each quadbin grid cell. */
case class RST_Quadbin_RasterToGridSum(
    tileExpr: Expression,
    ...
    override def prettyName: String = RST_Quadbin_RasterToGridSum.name
    override def replacement: Expression = rstInvoke(RST_Quadbin_RasterToGridSum, rasterType)
...
object RST_Quadbin_RasterToGridSum extends WithExpressionInfo {
    ...
    def execute(ds: Dataset, resolution: Int): Array[Array[(Long, Double)]] = {
        ...
        RST_Quadbin_RasterToGrid.execute(ds, resolution, sumF)
    }

    override def name: String = "gbx_rst_quadbin_rastertogridsum"

    override def builder(): FunctionBuilder = (c: Seq[Expression]) => new RST_Quadbin_RasterToGridSum(c(0), c(1))
