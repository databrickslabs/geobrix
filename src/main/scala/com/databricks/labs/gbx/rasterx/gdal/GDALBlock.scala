...
object GDALBlock {
    ...
    def apply(
        ...
        val noDataValue = BandAccessors.getNoDataValue(band)
        val rasterWidth = band.getXSize
        val rasterHeight = band.getYSize
        // Always read blockSize + stride pixels on every side
        // This is fine since kernel size is always much smaller than blockSize

        val padding = Padding(
          left = xOffset != 0,
          right = xOffset + xBlockSize < rasterWidth - 1, // not sure about -1
          top = yOffset != 0,
          bottom = yOffset + yBlockSize < rasterHeight - 1
        )

        val xo = Math.max(0, xOffset - stride)
        val yo = Math.max(0, yOffset - stride)

        val xs = getSize(xo, rasterWidth, xBlockSize, stride, padding.horizontalStrides)
        val ys = getSize(yo, rasterHeight, yBlockSize, stride, padding.verticalStrides)

        val block = Array.ofDim[Double](xs * ys)
        val maskBlock = Array.ofDim[Double](xs * ys)

        band.ReadRaster(xo, yo, xs, ys, block)
        band.GetMaskBand().ReadRaster(xo, yo, xs, ys, maskBlock)
