...
private[expressions] object CrsExpressionUtil {
    ...
    /** Parse a Catalyst-evaluated geometry value (Array[Byte] for BinaryType or UTF8String /
      * String for StringType) to a JTS Geometry. Returns null on parse failure — callers
      * treat null as a pass-through or no-op, never as an error.
      *
      * Note: `eval` here is the Spark Catalyst expression evaluation method, not JavaScript's
      * `eval`. The input comes from Spark's internal type system, not user-supplied code.
      */
    def parseGeom(geom: Any): Geometry = geom match {
        ...
        case s: String      => Try(JTS.fromWKT(s)).getOrElse(null)
        case _              => null
    }

    /** Return true if the input is a text medium (UTF8String or String). */
